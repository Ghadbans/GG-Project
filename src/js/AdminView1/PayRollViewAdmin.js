...
function PayRollViewAdmin() {
  ...
  const fetchData = async (page, searchTerm) => {
    if (navigator.onLine) {
      try {
        const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/payRoll-Information?page=${page + 1}&limit=${limit}&search=${encodeURIComponent(searchTerm.trim())}`)
  ...
  const syncOff = async () => {
    if (navigator.onLine) {
      ...
      for (const PayRollInfo of PayRollToSynced) {
        try {
          const res = await axios.post('https://globalgate-backend-production.up.railway.app/endpoint/create-payRoll', PayRollInfo)
          if (res) {
      ...
      for (const PayRollInfoUpdate of PayRollToSyncedUpdate) {
        try {
          await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-payRoll/${PayRollInfoUpdate._id}`, PayRollInfoUpdate)
  ...
  const handleDelete = async () => {
    try {
      const res = await axios.delete(`https://globalgate-backend-production.up.railway.app/endpoint/delete-payRoll/${DeleteId}`);
      if (res) {
        handleOpenModal();
      }
    } catch (error) {
  ...
  useEffect(() => {
    const fetchFunction = async () => {
      const deletePromises = selectedRows.map(async (idToDelete) => {
        return axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-payRoll/${idToDelete}`)
      })
      try {
  ...
  const handleDeleteMany = async (e) => {
    e.preventDefault()
    const deletePromises = selectedRows.map(async (idToDelete) => {
      return axios.delete(`https://globalgate-backend-production.up.railway.app/endpoint/delete-payRoll/${idToDelete}`)
    })
    try {
      const res = await Promise.all(deletePromises);
      if (res) {
  ...
  useEffect(() => {
    const fetchId = async () => {
      if (updateId !== null) {
        try {
          const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-payRoll/${updateId}`)
  ...
  const handleSubmitUpdateStatus = async (e) => {
    ...
    try {
      const res = await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-payRoll/${updateId}`, data)
      if (res) {
        handleOpenLoading();
      }
    } catch (error) {
