...
function PaymentView() {
  ...
  const fetchPayment = async (page, searchTerm) => {
    if (navigator.onLine) {
      try {
        const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/payment-Information?page=${page + 1}&limit=${limit}&search=${encodeURIComponent(searchTerm.trim())}`);
      ...
      const filtered = lowerSearch === '' ? offLineCustomer1 : offLineCustomer1.filter((item) =>
        ...
        (item.paymentNumber && item.paymentNumber.toString().includes(lowerSearch))
      );
  ...
  useEffect(() => {
    fetchPayment(page, debouncedSearchTerm);
  ...
  const syncOff = async () => {
    if (navigator.onLine) {
      ...
      for (const paymentInfo of paymentToSynced) {
        try {
          const res = await axios.post('https://globalgate-backend-production.up.railway.app/endpoint/create-payment', paymentInfo)
          if (res) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const resOverdue = await axios.get('https://globalgate-backend-production.up.railway.app/endpoint/invoice-Overdue')
  ...
  useEffect(() => {
    const fetchId = async () => {
      if (DeleteId) {
        if (navigator.onLine) {
          try {
            const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-payment/${DeleteId}`)
      ...
      if (relatedInvoice.length > 0) {
        if (navigator.onLine) {
          const getInvoice = relatedInvoice.map((row) => {
            return axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-invoice/${row.id}`)
          })
          try {
  ...
  useEffect(() => {
    const fetchCustomer = async () => {
      if (relatedCustomerID) {
        if (navigator.onLine) {
          try {
            const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-customer/${relatedCustomerID}`)
  ...
  const handleUpdateCredit = async () => {
    ...
    if (navigator.onLine) {
      try {
        await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-customer/${relatedCustomerID}`, data)
  ...
  const handleDelete = async (e) => {
    ...
    if (navigator.onLine) {
      try {
        const res = await axios.delete(`https://globalgate-backend-production.up.railway.app/endpoint/delete-payment/${DeleteId}`);
        if (res) {
          handleUpdateInvoice();
          handleUpdateCredit();
