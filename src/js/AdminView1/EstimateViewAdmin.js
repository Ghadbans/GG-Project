...
function EstimateViewAdmin() {
  ...
  const fetchData = async (page, searchTerm) => {
    if (navigator.onLine) {
      try {
        const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/estimate-Information?page=${page + 1}&limit=${limit}&search=${encodeURIComponent(searchTerm.trim())}`)
  ...
  const syncOff = async () => {
    if (navigator.onLine) {
      ...
      for (const estimateInfo of estimateToSynced) {
        try {
          const res = await axios.post('https://globalgate-backend-production.up.railway.app/endpoint/create-estimation', estimateInfo)
          if (res) {
      ...
      for (const estimateInfoUpdate of estimateToSyncedUpdate) {
        try {
          await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-estimation/${estimateInfoUpdate._id}`, estimateInfoUpdate)
  ...
  const handleDelete = async () => {
    try {
      const res = await axios.delete(`https://globalgate-backend-production.up.railway.app/endpoint/delete-estimation/${DeleteId}`);
      if (res) {
  ...
  useEffect(() => {
    const fetchFunction = async () => {
      const deletePromises = selectedRows.map(async (idToDelete) => {
        return axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-estimation/${idToDelete}`)
      })
      try {
  ...
  const handleDeleteMany = async (e) => {
    e.preventDefault()
    const deletePromises = selectedRows.map(async (idToDelete) => {
      return axios.delete(`https://globalgate-backend-production.up.railway.app/endpoint/delete-estimation/${idToDelete}`)
    })
    try {
      const res = await Promise.all(deletePromises);
      if (res) {
  ...
  useEffect(() => {
    if (updateId !== null) {
      axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-estimation/${updateId}`)
        .then(res => {
  ...
  const handleSubmitUpdateStatus = async (e) => {
    ...
    try {
      const res = await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-estimation/${updateId}`, data)
      if (res) {
        handleCreateComment();
        handleOpenLoading();
      }
