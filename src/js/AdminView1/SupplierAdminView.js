...
import { Table, IconButton, styled, TableBody, TableCell, TableHead, TableRow, Checkbox, TableContainer, Paper, Typography, Modal, Backdrop, Fade, Box, Autocomplete, TextField, Grid } from '@mui/material';
...
function SupplierAdminView() {
  ...
  const fetchData = async (page, searchTerm) => {
    if (navigator.onLine) {
      try {
        const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/supplier-Information?page=${page + 1}&limit=${limit}&search=${encodeURIComponent(searchTerm.trim())}`);
  ...
  const syncOff = async () => {
    if (navigator.onLine) {
      ...
      for (const customers of customertoSynChro) {
        try {
          const res = await axios.post('https://globalgate-backend-production.up.railway.app/endpoint/create-Supplier', customers)
          if (res) {
      ...
      for (const customersUpdate of customertoSynChroUpdate) {
        try {
          await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-Supplier/${customersUpdate._id}`, customersUpdate)
          handleOpenOffline();
        } catch (error) {
          console.log(error)
        }
  ...
  useEffect(() => {
    const fetchFunction = async () => {
      const deletePromises = selectedRows.map(async (idToDelete) => {
        return axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-Supplier/${idToDelete}`)
      })
      try {
  ...
  const handleDeleteMany = async (e) => {
    ...
    if (navigator.onLine) {
      const deletePromises = selectedRows.map(async (idToDelete) => {
        return axios.delete(`https://globalgate-backend-production.up.railway.app/endpoint/delete-Supplier/${idToDelete}`)
      })
      try {
        const res = await Promise.all(deletePromises);
        if (res) {
