...
function TewmViewAdmin() {
  ...
  const fetchData = async (page, searchTerm) => {
    if (navigator.onLine) {
      try {
        const res = await axios.get(`https://gg-project-productionn.up.railway.app/endpoint/employee-Information?page=${page + 1}&limit=${limit}&search=${encodeURIComponent(searchTerm.trim())}`)
  ...
  const syncOff = async () => {
    if (navigator.onLine) {
      ...
      for (const EmployeeInfo of EmployeeToSynced) {
        try {
          const res = await axios.post('https://gg-project-productionn.up.railway.app/endpoint/create-employee', EmployeeInfo)
          if (res) {
      ...
      for (const EmployeeInfoUpdate of EmployeeToSyncedUpdate) {
        try {
          await axios.put(`https://gg-project-productionn.up.railway.app/endpoint/update-employee/${EmployeeInfoUpdate._id}`, EmployeeInfoUpdate)
  ...
  useEffect(() => {
    const fetchFunction = async () => {
      const deletePromises = selectedRows.map(async (idToDelete) => {
        return axios.get(`https://gg-project-productionn.up.railway.app/endpoint/get-employee/${idToDelete}`)
      })
      try {
  ...
  const handleDeleteMany = async (e) => {
    e.preventDefault()
    const deletePromises = selectedRows.map(async (idToDelete) => {
      return axios.delete(`https://gg-project-productionn.up.railway.app/endpoint/delete-employee/${idToDelete}`)
    })
    try {
      const res = await Promise.all(deletePromises);
      if (res) {
