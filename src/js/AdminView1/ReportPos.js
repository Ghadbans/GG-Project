...
import { Skeleton,  Table, IconButton, styled, TableBody, TableCell, TableHead, TableRow, Checkbox, TableContainer, Paper, Typography, Modal, Box, Grid, FormControl, InputLabel, Select, MenuItem, Backdrop, Autocomplete, TextField, Tab, Card, CardContent, CardHeader  } from '@mui/material';
...
function ReportPos() {
  ...
  const TotalMonthPayment = invoiceFilteredByYear.reduce((acc, item) => {
  ...
  const TotalMonthPayment1 = invoiceFilteredByYear.reduce((acc, item) => {
    const month = dayjs(item.invoiceDate).format('MMMM');
    const year = dayjs(item.invoiceDate).format('YYYY');
    if (!acc[month]) {
      acc[month] = { year, month, amount: 0 }
  ...
  const TotalMonthDailyExpenses = invoiceFilteredByYear.reduce((acc, item) => {
    const month = dayjs(item.invoiceDate).format('MMMM');
    const year = dayjs(item.invoiceDate).format('YYYY');
    if (!acc[month]) {
                              ...
                              <CardContent style={{ display: 'block' }}>
                                ...
                                <section style={{ width: '100%', justifyContent: 'center' }}>
                                  <ul style={{ listStyleType: 'none' }}>
                                    <li style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}> <span><Square style={{ color: 'blue' }} /></span> <span style={{ color: 'blue' }}>Total Sales: $ {totalRevenue?.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</span></li>
                                    ...
                                    <li style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}> <span><Square style={{ color: 'green' }} /></span> <span style={{ color: 'green' }}>Net Profit: $ {(totalRevenue - totalCost)?.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</span></li>
