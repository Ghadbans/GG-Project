...
function ProjectViewAdmin() {
  ...
  const fetchData = async (page, searchTerm) => {
    if (navigator.onLine) {
      try {
        const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/project-Information?page=${page + 1}&limit=${limit}&search=${encodeURIComponent(searchTerm.trim())}`)
  ...
  const syncOff = async () => {
    if (navigator.onLine) {
      ...
      for (const projectInfo of projectsToSynced) {
        try {
          const res = await axios.post('https://globalgate-backend-production.up.railway.app/endpoint/create-projects', projectInfo)
          if (res) {
      ...
      for (const projectInfoUpdate of projectsToSyncedUpdate) {
        try {
          await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-projects/${projectInfoUpdate._id}`, projectInfoUpdate)
  ...
  useEffect(() => {
    const fetchFunction = async () => {
      const deletePromises = selectedRows.map(async (idToDelete) => {
        return axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-projects/${idToDelete}`)
      })
      try {
  ...
  const handleDeleteMany = async (e) => {
    e.preventDefault()
    const deletePromises = selectedRows.map(async (idToDelete) => {
      return axios.delete(`https://globalgate-backend-production.up.railway.app/endpoint/delete-projects/${idToDelete}`);
    })
    try {
      const res = await Promise.all(deletePromises);
      if (res) {
  ...
  const handleDelete = async () => {
    try {
      const deletePromises = selectedRows.map(async (idToDelete) => {
        await axios.delete(`https://globalgate-backend-production.up.railway.app/endpoint/delete-projects/${idToDelete}`)
      })
