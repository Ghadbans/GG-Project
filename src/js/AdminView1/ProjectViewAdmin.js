...
import { Add, Close, MailOutline, Person2Outlined, PersonOffRounded, Assignment as AssignmentIcon, HourglassEmpty as HourglassEmptyIcon, Autorenew as AutorenewIcon, Block as BlockIcon } from '@mui/icons-material';
...
function ProjectViewAdmin() {
  ...
  const fetchItems = async (page, searchTerm, filterField, filterValue, statusFilter) => {
    try {
      ...
      const res = await axios.get(`${ENDPOINT_URL}/project-Information?page=${page + 1}&limit=${limit}&search=${encodeURIComponent((searchTerm || '').trim())}&filterField=${encodeURIComponent((filterField || '').trim())}&filterValue=${encodeURIComponent((filterValue || '').trim())}${statusParam}`);
  ...
  if (isMobile) {
    return loadingData ? (
      ...
      <Box sx={{ width: '100%', py: 1 }}>
        <MobileCardList type="projects" data={project} searchPlaceholder="Search projects..." />
      </Box>
    );
  }
      ...
      <Box sx={{ display: 'flex' }}>
        ...
        >
          ...
          <Container maxWidth="none" sx={{ mt: 0.5, px: { xs: 1, sm: 2 } }} >
            ...
            >
              {[
                { key: 'ALL', label: 'ALL PROJECTS', count: statusCounts.all, color: '#30368a', bgLight: '#eef2ff', icon: <AssignmentIcon sx={{ fontSize: 15, color: '#30368a' }} /> },
              ...
              loadingData ? <div >
                ...
                <div>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.5, mt: 0.25 }}>
                    <Box>
                      {project.length > 0 && selectedRows.length > 1 && selectedRows.length < project.length && (
                        <button disabled={user.data.role !== 'CEO'} onClick={handleOpenAll} className='btnCustomer2'>Delete multiple</button>
                      )}
                      {project.length > 0 && selectedRows.length === project.length && (
                    ...
                    {!isNativeMobile() && (
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <ViewTooltip>
                          <span>
                            <IconButton disabled={ProjectInfoC.length === 0} size="small">
