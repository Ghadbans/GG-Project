...
function PurchaseOrderViewAdmin() {
  ...
  const fetchItems = async (page, searchTerm, filterField, filterValue) => {
    try {
      const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/purchaseOrder-Information?page=${page + 1}&limit=${limit}&search=${encodeURIComponent(searchTerm.trim())}&filterField=${encodeURIComponent(filterField.trim())}&filterValue=${encodeURIComponent(filterValue.trim())}`);
  ...
  const fetchAndSaveData = async () => {
    try {
      const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/purchaseOrder`);
  ...
  useEffect(() => {
    const fetchData2 = async () => {
      ...
      if (idView !== null) {
        if (navigator.onLine) {
          try {
            const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-purchaseOrder/${idView}`)
  ...
  useEffect(() => {
    const fetchId = async () => {
      if (DeleteId !== null) {
        try {
          const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-purchaseOrder/${DeleteId}`)
  ...
  const onStatusUpdate = async (e, idInfo) => {
    ...
    try {
      ...
      const res = await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-purchaseOrder/${idInfo}`, data);
      if (res) {
        setUpdateS(true);
  ...
  const handleDeleteUpdate = async (e) => {
    e.preventDefault()
    try {
      const res = await axios.delete(`https://globalgate-backend-production.up.railway.app/endpoint/delete-purchaseOrder/${DeleteId}`);
      if (res) {
        handleDeleteOpenLoading();
