...
function MaintenanceViewAdmin() {
  ...
  const fetchItems = async (page, searchTerm) => {
    if (navigator.onLine) {
      try {
        const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/maintenance-Information?page=${page + 1}&limit=${limit}&search=${encodeURIComponent(searchTerm.trim())}`);
  ...
  useEffect(() => {
    if (updateId !== null) {
      axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-maintenance/${updateId}`)
        .then(res => {
  ...
  const handleSubmitUpdateStatus = async (e) => {
    ...
    try {
      const res = await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-maintenance/${updateId}`, data)
      if (res) {
        handleOpenLoading();
        handleCreateComment();
      }
  ...
  const handleDelete = async () => {
    try {
      const res = await axios.delete(`https://globalgate-backend-production.up.railway.app/endpoint/delete-maintenance/${DeleteId}`);
      if (res) {
  ...
  useEffect(() => {
    const fetchFunction = async () => {
      const deletePromises = selectedRows.map(async (idToDelete) => {
        return axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-maintenance/${idToDelete}`)
      })
      try {
  ...
  const handleDeleteMany = async (e) => {
    e.preventDefault()
    const deletePromises = selectedRows.map(async (idToDelete) => {
      return axios.delete(`https://globalgate-backend-production.up.railway.app/endpoint/delete-maintenance/${idToDelete}`)
    })
    try {
      const res = await Promise.all(deletePromises);
      if (res) {
