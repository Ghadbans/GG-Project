...
function EmployeePlaningFormUpdate() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`${ENDPOINT_URL}/get-planing/${id}`)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setEmployee(employeeResponse.data?.data?.filter((row) => row.department?.toUpperCase() !== 'FACTORY' && employeeAttendance?.find((row2) => row2.ID === row._id)));
      } catch (error) {
        console.log(error)
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(`${ENDPOINT_URL}/update-planing/${id}`, data);
      if (res) {
