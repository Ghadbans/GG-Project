...
if (navigator.onLine) {
      try {
        const res = await axios.get('https://gg-project-productionn.up.railway.app/endpoint/planing')
  ...
  const syncOff = async () => {
    if (navigator.onLine) {
     ...
     for(const Items of ItemToSynChro){
      try {
         const res = await axios.post('https://gg-project-productionn.up.railway.app/endpoint/create-planing',Items)
        if (res) {
      ...
      for(const ItemsUpdate of ItemToSynChroUpdate){
        try {
           await axios.put(`https://gg-project-productionn.up.railway.app/endpoint/update-planing/${ItemsUpdate._id}`,ItemsUpdate)
           handleOpenOffline();
        } catch (error) {
          console.log(error)
        }
...
useEffect(()=>{
  const fetchId = async () => {
    if (DeleteId !== null) {
      try {
        const res = await  axios.get(`https://gg-project-productionn.up.railway.app/endpoint/get-planing/${DeleteId}`)
...
const handleDeleteUpdate = async (e) => {
  e.preventDefault()
  try {
    const res = await axios.delete(`https://gg-project-productionn.up.railway.app/endpoint/delete-planing/${DeleteId}`);
    if (res) {
