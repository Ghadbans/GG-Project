...
function EmployeeUpdateView() {
 ...
 useEffect(()=>{
  ...
  useEffect (() => {
    const fetchData = async () => {
     if (navigator.onLine) {
       try {
         const res = await axios.get(`https://gg-project-productionn.up.railway.app/endpoint/get-employee/${id}`)
 ...
 {/** Loading End */}
  const handleSubmit = async (e) => {
     ...
     if (navigator.onLine) {
       try {
         const res = await axios.put(`https://gg-project-productionn.up.railway.app/endpoint/update-employee/${id}`,data)
         if (res) {
           handleOpen();
