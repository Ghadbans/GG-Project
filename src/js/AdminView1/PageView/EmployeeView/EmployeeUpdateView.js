...
function EmployeeUpdateView() {
  ...
  useEffect(() => {
    const handleFetch = async () => {
      try {
        const resItemUnit = await axios.get(`${ENDPOINT_URL}/department`)
        const currentBranch = localStorage.getItem('selectedBranch') || 'HQ';
        const filteredUnits = (resItemUnit.data?.data || []).filter(d => !d.branchId || d.branchId === currentBranch);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setIdentifierNumber(Number(res.data?.data?.identifierNumber || res.data?.identifierNumber || 0));
        setExpireDate(res.data.data.expireDate);
        setSalary(res.data.data.salary);
        setDescription(res.data.data.description);
        setBankName(res.data.data.bankName);
        setBankAccountNumber(Number(res.data?.data?.bankAccountNumber || res.data?.bankAccountNumber || 0));
  ...
  const handleSubmitCategory = async (e) => {
    ...
    try {
      const res = await axios.post(`${ENDPOINT_URL}/create-department`, data);
      if (res) {
        handleOpenModal();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(`${ENDPOINT_URL}/update-employee/${id}`, data)
      if (res) {
        handleOpen();
        handleCreateComment();
      }
          ...
          <Box sx={{ ...style, width: '90%', maxWidth: 400, borderRadius: 4, textAlign: 'center', p: 3 }}>
            {loading ? <Loader /> : (
              <div>
                ...
                <Typography variant="h6" sx={{ fontWeight: 800 }}>Employee Updated Successfully</Typography>
