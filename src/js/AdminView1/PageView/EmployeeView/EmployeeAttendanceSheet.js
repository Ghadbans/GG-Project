     ...
     const syncOff = async () => {
      if (navigator.onLine) {
       ...
       for(const EmployeeInfo of EmployeeToSynced){
        try {
           await axios.post('https://globalgate-backend-production.up.railway.app/endpoint/create-employeeattendance',EmployeeInfo)
        ...
        for(const EmployeeInfoUpdate of EmployeeToSyncedUpdate){
          try {
             await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-employeeattendance/${EmployeeInfoUpdate._id}`,EmployeeInfoUpdate)
...
useEffect(()=>{
  const fetchData2 = async () => {
   ...
   if (navigator.onLine) {
     try {
         const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-employeeattendance/${idView}`)
