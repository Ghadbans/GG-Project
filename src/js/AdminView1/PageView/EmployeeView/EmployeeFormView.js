...
function EmployeeFormView() {
  ...
  const apiUrl = `${ENDPOINT_URL}/create-employee`;
  ...
  useEffect(() => {
    const handleFetch = async () => {
      try {
        ...
        const lastEmpRes = await axios.get(`${ENDPOINT_URL}/get-last-saved-employee`);
        if (lastEmpRes && lastEmpRes.data && lastEmpRes.data.employeeId) {
          const lastIdStr = String(lastEmpRes.data.employeeId);
  ...
  const handleImageSubmit = async () => {
    ...
    try {
      const res = await axios.post(`${ENDPOINT_URL}/upload-image`, formData, {
        headers: {
