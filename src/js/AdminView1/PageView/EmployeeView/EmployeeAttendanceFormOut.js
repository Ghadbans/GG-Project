...
function EmployeeAttendanceFormOut() {
  ...
  useEffect(() => {
    const fetchEmployee = async () => {
      try {
        const resEmp = await axios.get(`${ENDPOINT_URL}/employee`)
        const factoryIds = resEmp.data?.data?.filter(emp => emp.department?.toUpperCase() === 'FACTORY').map(emp => emp._id)

        const res = await axios.get(`${ENDPOINT_URL}/employeeattendance`)

        const nonFactoryData = res.data?.data?.filter(att => !factoryIds.includes(att.id))

        setEmployee(nonFactoryData.filter((row) => dayjs(row.timeIn).format('DD/MM/YYYY') === dayjs(dateAttendance).format('DD/MM/YYYY')))
      } catch (error) {
        console.log(error)
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const saveAttendance = employee.map((row) => {
      return axios.put(`${ENDPOINT_URL}/update-employeeattendance/${row._id}`, {
        timeIn: row.timeIn,
        timeOut: row.timeOut,
