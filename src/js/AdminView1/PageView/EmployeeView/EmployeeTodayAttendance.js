  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const nonFactoryAttendance = res.data?.data?.filter(att => !factoryIds.includes(att.id))
        const nonFactoryPlaning = resPlaning.data?.data?.filter(plan => !factoryIds.includes(plan.employeeID))
