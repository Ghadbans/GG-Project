 ...
 const [nameDelete,setNameDelete] = useState('')
  useEffect(()=>{
    const fetchData2 = async () => {
      ...
      if (DeleteId !== null) {
      try {
          const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-grantAccess/${DeleteId}`)
  ...
  const handleDelete = async (e) => {
    e.preventDefault()
    try {
      const res = await axios.delete(`https://globalgate-backend-production.up.railway.app/endpoint/delete-grantAccess/${DeleteId}`);
      if (res) {
