...
function ItemInformationVIew() {
  ...
  const handleFetchSummaryData = async () => {
    ...
    try {
      ...
      await Promise.all([
        fetchFiltered(
          `https://globalgate-backend-production.up.railway.app/endpoint/itemOut/item/${id}`,
          setItemOut,
          (row) => ({ ...row, itemsQtyArray: row.itemsQtyArray.filter((Item) => Item.itemName && Item.itemName._id === id && parseFloat(Item.newItemOut) > 0) })
        ),
        fetchFiltered(
          `https://globalgate-backend-production.up.railway.app/endpoint/itemPurchase/item/${id}`,
          setItemPurchase,
          (row) => ({ ...row, items: row.items.filter((Item) => Item.itemName && Item.itemName._id === id) })
        ),
        fetchFiltered(
          `https://globalgate-backend-production.up.railway.app/endpoint/pos/item/${id}`,
          setPosOut,
          (row) => ({ ...row, items: row.items.filter((Item) => Item.itemName && Item.itemName._id === id && parseFloat(Item.itemQty) > 0) })
        ),
        fetchFiltered(
          `https://globalgate-backend-production.up.railway.app/endpoint/itemReturn/item/${id}`,
          setItemReturn,
          (row) => ({ ...row, itemsQtyArray: row.itemsQtyArray.filter((Item) => Item.itemName && Item.itemName._id === id && parseFloat(Item.newItemOut) > 0) })
        )
      ]);
  ...
  const fetchStock = async () => {
    try {
      const res = await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-item/${id}`, {
        itemQuantity: stock
      })
    } catch (error) {
      console.log(error)
  ...
  const handleImageSubmit = async (e) => {
    ...
    try {
      await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/upload-image-item/${id}`, formData, {
        headers: {
  ...
  useEffect(() => {
    const fetchData2 = async () => {
      ...
      if (idView2 !== null) {
        if (navigator.onLine) {
          try {
            const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-itemOut/${idView2}`)
