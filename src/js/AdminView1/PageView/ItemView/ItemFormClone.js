...
function ItemFormClone() {
  ...
  useEffect(() => {
    const handleFetch = async () => {
      try {
        const resItemUnit = await axios.get(`${ENDPOINT_URL}/itemUnit`)
        setUnitInfo(resItemUnit.data.data);
        const resCode = await axios.get(`${ENDPOINT_URL}/itemCode`)
  ...
  const handleChangeUnits = (newValue) => {
