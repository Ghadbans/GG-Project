...
function ItemForm() {
  ...
  useEffect(() => {
    const handleFetchInitialData = async () => {
      try {
        const [resItemUnit, resCode] = await Promise.all([
          axios.get(`${ENDPOINT_URL}/itemUnit`),
          axios.get(`${ENDPOINT_URL}/itemCode`)
        ]);
  ...
  useEffect(() => {
    const fetchNumber = async () => {
      if (newCode) {
        try {
          const res = await axios.get(`${ENDPOINT_URL}/get-last-saved-item/${newCode}`)
  ...
  const handleSubmitCategory = async (e) => {
    ...
    try {
      const res = await axios.post(`${ENDPOINT_URL}/create-itemUnit`, data);
      if (res) {
        handleOpenModal();
    ...
    return (
      <Box sx={{ width: '100%', minHeight: '100vh', backgroundColor: '#F8FAFC', pb: 12 }}>
        ...
        <Box sx={{ p: 2 }}>
              ...
              <Grid item xs={6}>
                <FormControl fullWidth size="small">
                  ...
                  <Select
                    ...
                    label="Unit"
                    onChange={(e) => {
                      const sel = unitInfo.find(u => u.itemUnit === e.target.value);
                      setUnit(sel || { itemUnit: e.target.value });
                    }}
                  >
                    {unitInfo.map((u) => (
          ...
          >
            {saving === 'true' ? 'Saving Item...' : 'Save Item'}
          </Button>
        </Box>
          ...
          <Box sx={{ ...style, width: '90%', maxWidth: 400, borderRadius: 4, textAlign: 'center', p: 3 }}>
            {loading ? <Loader /> : (
              <div>
                ...
                <Typography variant="h6" sx={{ fontWeight: 800 }}>Item Saved Successfully</Typography>
        ...
        <Box sx={{ ...style, width: 500 }}>
          ...
          <Grid container sx={{ alignItems: 'center', padding: '15px' }} spacing={2}>
            <Grid item xs={12} sx={{ textAlign: 'center' }}>
              <Typography>Do you want to stop creating Item ? </Typography>
