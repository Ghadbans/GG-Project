...
The GLOBAL GATE Team`;
  ...
  const handleAddToEstimate = (shopItem) => {
        ...
        <Box sx={{ p: 2 }}>
          ...
          <Card sx={{ borderRadius: 3.5, p: 2.5, mb: 2, backgroundColor: '#ffffff', boxShadow: '0 2px 10px rgba(0,0,0,0.04)' }}>
            ...
            <Autocomplete
              size="small"
              options={customer || []}
              getOptionLabel={(option) => option?.customerName || option?.Customer || option?.name || ''}
              ...
              renderInput={(params) => (
                <TextField {...params} label="Select Customer *" size="small" fullWidth sx={{ mb: 2 }} />
              )}
            />
          ...
          <Box sx={{ position: 'fixed', bottom: 0, left: 0, right: 0, p: 2, backgroundColor: '#ffffff', borderTop: '1px solid #E2E8F0', zIndex: 1000, boxShadow: '0 -2px 10px rgba(0,0,0,0.05)' }}>
            ...
            >
              {saving === 'true' ? 'Saving Quotation...' : `Create Quotation (${formatMoney(totalInvoice)})`}
            </Button>
          </Box>
        </Box>
      </Box>
