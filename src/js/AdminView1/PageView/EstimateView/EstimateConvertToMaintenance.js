...
function EstimateConvertToMaintenance() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (resLast.data && resLast.data.serviceNumber) {
          setServiceNumber(parseInt(resLast.data?.serviceNumber || 0) + 1);
        } else {
          setServiceNumber(1);
        }
    ...
    const fetchEstimateData = async () => {
      if (id) {
        try {
          ...
          let eNum = estimate.estimateNumber;
          if (eNum === undefined && estimate.estimateName) {
            const match = String(estimate.estimateName).match(/\d+$/);
  ...
  const fetchShop = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${ENDPOINT_URL}/item-shop?page=${shopPage}&limit=20&search=${encodeURIComponent(shopSearch)}&branchId=${localStorage.getItem('selectedBranch') || 'HQ'}`)
  ...
  const handleCreateNotification = async (ReferenceInfo, ReferenceInfoNumber) => {
    ...
    try {
      await axios.post(`${ENDPOINT_URL}/create-notification`, { ...data, branchId: localStorage.getItem("selectedBranch") || "HQ" })
    } catch (error) {
      console.log(error)
    }
  }
  const handleSubmitUpdateStatus = async (ReferenceInfo2) => {
    ...
    try {
      await axios.put(`${ENDPOINT_URL}/update-estimation/${id}`, { ...updateData, branchId: localStorage.getItem("selectedBranch") || "HQ" })
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res) {
        ...
        await handleSubmitUpdateStatus(ReferenceInfo);
              ...
              <form onSubmit={handleSubmit}>
                ...
                <Grid container style={{ alignItems: 'center', padding: '15px' }} spacing={1} component={Paper}>
                  ...
                  <Grid item xs={9}>
                    <Autocomplete
                      disableClearable
                      value={customer.length > 0 && customerName?._id ? customer.find(c => c._id === customerName._id) || null : null}
                      options={customer}
                  ...
                  <Grid item xs={6} key={item._id}>
                    <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                      <CardMedia
                        component="img"
                        height="120"
                        image={item.data ? `data:${item.contentType};base64,${item.data}` : undefined}
                        alt={item.itemName}
...
export default EstimateConvertToMaintenance
