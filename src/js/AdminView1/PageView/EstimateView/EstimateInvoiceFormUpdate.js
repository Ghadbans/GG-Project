...
function EstimateInvoiceFormUpdate() {
  const { id } = useParams();
  const { isLocked, lockConfig, lockError, forceRelease } = useDocumentLock(id, 'estimation');
        ...
        <Box sx={{ p: 2 }}>
          ...
          <Card sx={{ borderRadius: 3.5, p: 2.5, mb: 2, backgroundColor: '#ffffff', boxShadow: '0 2px 10px rgba(0,0,0,0.04)' }}>
            ...
            <Box sx={{ mt: 2, pt: 2, borderTop: '1px dashed #E2E8F0' }}>
              <Autocomplete
                ...
                onChange={(e, val) => {
                  if (val) {
                    const itemRate = Number(val.rate || val.sellingPrice || val.price || 0);
                    SetItems(prev => [...prev, {
                      idRow: v4(),
          ...
          <Card sx={{ borderRadius: 3.5, p: 2.5, mb: 2, backgroundColor: '#ffffff', boxShadow: '0 2px 10px rgba(0,0,0,0.04)' }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Typography variant="subtitle1" sx={{ fontWeight: 800, color: '#1E293B' }}>Total Quotation:</Typography>
              <Typography variant="h6" sx={{ fontWeight: 900, color: '#10B981' }}>{formatMoney(totalInvoice)}</Typography>
            </Box>
          </Card>
          ...
          <Box sx={{ position: 'fixed', bottom: 0, left: 0, right: 0, p: 2, backgroundColor: '#ffffff', borderTop: '1px solid #E2E8F0', zIndex: 1000, boxShadow: '0 -2px 10px rgba(0,0,0,0.05)' }}>
            <Button
              fullWidth
              variant="contained"
              size="large"
              ...
              sx={{
                ...
                textTransform: 'none',
                fontSize: '1rem',
                '&:hover': { backgroundColor: '#202560' }
              }}
            >
              {loading ? 'Saving...' : `Update Quotation (${formatMoney(totalInvoice)})`}
            </Button>
          </Box>
        </Box>
      </Box>
