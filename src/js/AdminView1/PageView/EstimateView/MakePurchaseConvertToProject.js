...
function MakePurchaseConvertToProject() {
    ...
    useEffect(()=>{
      const fetchlastNumber = async () => {
        try {
          const res = await axios.get('https://globalgate-backend-production.up.railway.app/endpoint/get-last-saved-purchase')
...
const handleCreateItem = (newItem)=> {
  ...
  const [saving,setSaving] = useState('')
    const handleConvertInvoiceToPurchase = async (e) => {
      ...
      try{
        const res = await axios.post('https://globalgate-backend-production.up.railway.app/endpoint/create-purchase',data);
        if (res) {
           // Open Loading View
