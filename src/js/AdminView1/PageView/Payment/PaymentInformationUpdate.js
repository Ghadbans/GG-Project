...
function PaymentInformationUpdate() {
  ...
  useEffect(() => {
    const fetchlastNumber = async () => {
      try {
        const res = await axios.get(`${ENDPOINT_URL}/get-payment/${id}`)
  ...
  useEffect(() => {
    const fetchlastNumber = async () => {
      try {
        ...
        setPaymentNumber(Number(res.data?.data?.paymentNumber || res.data?.paymentNumber || 0));
        setCreditOld(res.data.data.remaining);
      ...
      try {
        ...
        if (customerId !== null) {
          if (reason === "Invoice") {
            ...
            const exProject = filterData2.filter((row) => !invoice1.some((row1) => row1.id === row._id))
              .map((row) => ({
                id: row._id,
                customerName: row.customerName,
                total: 0,
  ...
  const remaining = Math.round(remainingValue * 100) / 100
  ...
  const invoiceUpdate = (reason === "Invoice" && invoice) ? invoice.map((row) => {
    const correspondingId = oldInvoice?.find((row2) => row2._id === row.id)
    if (!correspondingId) return null; // Safety check

    let difference = 0
    let total = 0
    const difference1 = Math.round((parseFloat(row.balanceDue) - parseFloat(row.total)) * 100) / 100;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(`${ENDPOINT_URL}/update-payment/${id}`, data);
      if (res) {
                ...
                <Grid container style={{ alignItems: 'center', padding: '15px' }} spacing={2} component={Paper}>
                  ...
                  <Grid item xs={6}>
                    <Typography variant="body2" color="textSecondary">
                      Customer Current Credit: <strong>${oldCredit?.toFixed(2)}</strong>
                    </Typography>
                  </Grid>
                  <Grid item xs={7}>
