...
import { Checkbox, FormControlLabel, MenuItem, Grid, IconButton, Paper, TextField, FormControl, InputLabel, Select, Typography, styled, Box, Autocomplete, Modal, Backdrop, TableContainer, OutlinedInput, InputAdornment, Divider, Card, CardContent, CardMedia, Pagination, Button, SwipeableDrawer, Drawer as SideDrawer, Avatar } from '@mui/material';
...
function MaintenanceFormView() {
  ...
  const filterEmployee = employee.filter((row) => (row.department === 'TECHNICIAN' || (row.department || '').toUpperCase() === 'TECHNICIAN') && row.Status !== 'Fired' && row.Status !== 'Resign' && row.Status !== 'Suspended');
