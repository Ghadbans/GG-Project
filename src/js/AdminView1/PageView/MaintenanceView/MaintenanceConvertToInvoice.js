...
const handleCreateItem = (newItem)=> {
  ...
  useEffect(()=> {
    ...
    const handleSubmitStatusUpdate = async (ReferenceInfo) => {
        ...
        try {
          await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-maintenance/${id}`,data)
        } catch (error) {
          console.log(error)
        }
     }
    ...
    const data = {
      ...
      try{
        const res = await axios.post('https://globalgate-backend-production.up.railway.app/endpoint/create-invoice',data);
        if (res) {
