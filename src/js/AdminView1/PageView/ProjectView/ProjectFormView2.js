...
const ProjectFormView2 = ({onCreateOption, onClose,onId,onEstimate}) => {
  ...
  useEffect(()=> {
    ...
    useEffect (() => {
      ...
      if (navigator.onLine) {
           try {
             const resInvoice = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-invoice/${onId}`)
       ...
       if (navigator.onLine) {
         try {
           const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-estimation/${onEstimate}`)
