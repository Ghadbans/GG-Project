...
function ProjectUpdateView() {
  ...
  const { id } = useParams();
  const { isLocked, lockConfig, lockError, forceRelease } = useDocumentLock(id, 'project');
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`${ENDPOINT_URL}/get-projects/${id}`)
        ...
        setProjectNumber(Number(res.data?.data?.projectNumber || res.data?.projectNumber || 0));
  ...
  const handleCreateComment = async () => {
    const data = {
      idInfo: id,
      person: `${user.data.userName} Modify P-${String(projectNumber).padStart(6, '0')}`,
      reason,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const currentRes = await axios.get(`${ENDPOINT_URL}/get-projects/${id}`);
      const currentProject = currentRes.data.data;
      ...
      const res = await axios.put(`${ENDPOINT_URL}/update-projects/${id}`, updatedData);
      if (res) {
        handleCreateComment();
        handleOpen();
      }
  ...
  if (isLocked) {
    const isCEO = (user?.data?.role || '').toUpperCase() === 'CEO' || (user?.data?.userName || '').toUpperCase() === 'GG';
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', flexDirection: 'column', backgroundColor: 'rgba(0,0,0,0.85)', color: 'white', position: 'fixed', top: 0, left: 0, width: '100%', zIndex: 9999 }}>
        ...
        <p style={{ color: '#ccc', marginBottom: '20px' }}>
          {isCEO 
            ? 'Please wait until they are finished, or force unlock if you need immediate access.' 
            : 'Please wait until they are finished editing this document.'}
        </p>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <TextField
                  ...
                  size="small"
                  label="Customer Name"
                  value={customerName?.customerName || customerName?.Customer || ''}
                />
              </Grid>
          ...
          <Box sx={{ ...style, width: '90%', maxWidth: 400, borderRadius: 4, textAlign: 'center', p: 3 }}>
            {loading ? <Loader /> : (
              <div>
                ...
                <Typography variant="h6" sx={{ fontWeight: 800 }}>Project Updated Successfully</Typography>
