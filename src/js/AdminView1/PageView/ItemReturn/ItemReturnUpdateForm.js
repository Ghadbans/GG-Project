...
const ItemThumbnail = ({ itemId, initialData, initialType }) => {
  ...
  useEffect(() => {
    const fetchImage = async () => {
      ...
      if (navigator.onLine) {
        try {
          const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-item/${itemId}`);
...
function ItemReturnUpdateForm() {
  ...
  useEffect(() => {
    const handleFetch = async () => {
      try {
        const resItemOut = await axios.get('https://globalgate-backend-production.up.railway.app/endpoint/get-last-saved-itemReturn')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('https://globalgate-backend-production.up.railway.app/endpoint/create-itemReturn', data);
      if (res) {
        // Open Loading View
