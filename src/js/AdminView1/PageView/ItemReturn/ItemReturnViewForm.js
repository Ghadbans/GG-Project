...
function ItemReturnViewForm() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-itemReturn/${id}`)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-itemReturn/${id}`, data)
      if (res) {
        // Open Loading View
        //handleUpdateQty();
        handleUpdateInfo();
