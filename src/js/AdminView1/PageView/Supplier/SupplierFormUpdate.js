...
function SupplierFormUpdate() {
    ...
    useEffect(()=>{
        const fetchData = async () => {
         if (navigator.onLine) {
           try {
             const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-Supplier/${id}`)
            ...
            const resLocal = await db.supplierSchema.get({_id:id})
            setIndexID(resLocal.id);
            setSupplierName(resLocal.supplierName);
            setStoreName(resLocal.storeName);
            setCustomerPhone1(resLocal.customerPhone1);
            setCustomerPhone2(resLocal.customerPhone2);
            setAddress(resLocal.address);
      ...
      const handleCreateComment = async () => {
        const data = {
          idInfo:id,
          person:user.data.userName + ' Modify ' + storeName,
          reason,
      ...
      const handleSubmitEdit = async(e) => {
         ...
         if (navigator.onLine) {
           try {
             const res = await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-Supplier/${id}`,data)
             if (res) {
               handleCreateComment();
               handleOpen();
             }
          ...
          await db.supplierSchema.update(indexId,data)
          handleOpen();
        }
      };
  return (
