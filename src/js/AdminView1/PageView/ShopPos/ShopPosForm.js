...
import { MenuItem, Grid, IconButton, Paper, TextField, FormControl, InputLabel, Select, Typography, styled, Box, Autocomplete, Modal, Backdrop, TableContainer, OutlinedInput, InputAdornment, Divider, Checkbox, FormControlLabel, Card, CardMedia, CardContent, Pagination, Button } from '@mui/material'
...
function ShopPosForm() {
  ...
  const fetchItems = async (page, searchTerm) => {
    ...
    if (navigator.onLine) {
      try {
        const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/item-shop?page=${page}&limit=48&search=${encodeURIComponent(searchTerm.trim())}&sort=-_id`);
  ...
  const [note, setNote] = useState("Thank you for your visit. Goods sold are neither returned or exchanged.");
  const dateComment = new Date()
  const [rate, setRate] = useState(0);

  const Create = user.data.userName

  useEffect(() => {
    const fetchlastNumber = async () => {
      if (navigator.onLine) {
        try {
          const resRate = await axios.get('https://globalgate-backend-production.up.railway.app/endpoint/rateReturn')
          resRate.data.data.map((row) => setRate(row.rateR))
          const res = await axios.get('https://globalgate-backend-production.up.railway.app/endpoint/get-last-saved-pos')
  ...
  const handleAddToInvoice = (selectedItem) => {
    ...
    SetItems(prevItems => {
      const existingItemIndex = prevItems.findIndex(item => item.itemName._id === selectedItem._id);
      ...
      if (existingItemIndex >= 0) {
        ...
        const newRate = selectedItem.itemSellingPrice * rate;
        const newCost = selectedItem.itemCostPrice * rate;
  ...
  const { subTotal, tax, totalInvoice, balanceDue, credit, TotalAmountPaid, remaining } = useMemo(() => {
    ...
    const usdConvert = totalUSD * rate;
    const totalPaid = Number(usdConvert) + Number(totalFC);
    const newBalance = Math.round((TWTVA - totalPaid) * 100) / 100;

    // Credit Logic
    const creditI = Math.abs(newBalance);
    const usdConvert1 = creditUsd * rate;
    const totalPaid1 = Number(usdConvert1) + Number(creditFC);
    const rest = Math.round((creditI - totalPaid1) * 100) / 100;
    const totalPaidInfo = totalPaid - totalPaid1;
  ...
  const handleSubmit = async (e) => {
    ...
    if (navigator.onLine) {
      try {
        const res = await axios.post('https://globalgate-backend-production.up.railway.app/endpoint/create-pos', data);
        if (res) {
          ...
          <Container maxWidth="none" sx={{ mt: 4 }} >
            <div >
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} md={7} sx={{ height: '85vh', overflowY: 'auto', overflowX: 'hidden', pr: 1 }}>
                      ...
                      <Grid container style={{ alignItems: 'center' }} spacing={2}>
                        <Grid item xs={12}>
                          <Autocomplete
                            ...
                            renderInput={(params) => <TextField {...params} label="Customer Name" required sx={{ '& .MuiInputBase-input': { fontSize: '15px' }, '& .MuiInputLabel-root': { fontSize: '15px' } }} />}
                          />
                        </Grid>
                          ...
                          <LocalizationProvider dateAdapter={AdapterDayjs}>
                            <DemoContainer components={['DatePicker']}>
                              <DatePicker
                                ...
                                slotProps={{ textField: { sx: { '& .MuiInputBase-input': { fontSize: '15px' }, '& .MuiInputLabel-root': { fontSize: '15px' } } } }}
                                format="DD/MM/YYYY"
                              />
                            </DemoContainer>
                        ...
                        <Grid item xs={12}>
                          ...
                          </div>
                          {
                            user.data.role === 'CEO' ?
                                                ...
                                                >
                                                  {
                                                    Item.newDescription !== undefined ? (
                                                      <>
                                                        <td {...provided.dragHandleProps} style={{ textAlign: 'center', cursor: 'grab', fontSize: '15px', fontWeight: 'bold' }}>{i + 1}</td>
                                                        <td colSpan={6}><TextField
                                                          ...
                                                          sx={{ width: '100%', backgroundColor: 'white', fontSize: 15 }}
                                                        /></td>
                                                        <td >
                                                            ...
                                                            Item.itemName.itemName ? (
                                                              ...
                                                              <div style={{ display: 'flex', alignItems: 'center' }}>
                                                                <Autocomplete
                                                                  ...
                                                                  sx={{ width: '300px', backgroundColor: 'white', '& .MuiInputBase-input': { fontSize: '15px' } }}
                                                                />
                                                              </div>
                                                            )
                                                          }
                              ...
                              :
                              (
                                <div style={{ display: 'flex', alignItems: 'center', gap: '20px', overflowX: 'auto', width: '100%' }}>
                                  <DragDropContext onDragEnd={handleDragEnd}>
                                    <table className='tableInfo10'>
                                      <thead>
                                        <tr>
                                                      ...
                                                      Item.newDescription !== undefined ? (
                                                        <>
                                                          ...
                                                          <td colSpan={5}><TextField
                                                            ...
                                                            size="small"
                                                            disabled={user.data.role === 'User'}
                                                            sx={{ width: '100%', backgroundColor: 'white', '& .MuiInputBase-input': { fontSize: '15px' } }}
                                                          /></td>
                                                          <td >
                                                            <LightTooltip title="Delete" sx={{}}>
                                                              <IconButton onClick={() => deleteItem(Item.idRow)} >
                                                        ...
                                                        <>
                                                          ...
                                                          <td id='amountTotalInvoice' style={{ fontSize: '15px' }}>{Item.itemAmount.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</td>
                                                          <td >
                                                            <LightTooltip title="Delete" sx={{}}>
                                                              <IconButton onClick={() => deleteItem(Item.idRow)} >
                                                                <DeleteIcon style={{ cursor: 'pointer', color: 'red' }} />
                        ...
                        <Grid item xs={12}>
                          <div style={{ display: 'flex', gap: '20px', justifyContent: 'space-between', alignItems: 'center' }}>
                            <TextField
                              ...
                              sx={{ width: '50%', backgroundColor: 'white', '& .MuiInputBase-input': { fontSize: '15px' }, '& .MuiInputLabel-root': { fontSize: '15px' } }}
                            />
                          </div>
                        </Grid>
                        <Grid item xs={12}>
                ...
                <Grid item xs={12} md={5} sx={{ height: '85vh', overflowY: 'auto' }}>
                  <Paper sx={{ padding: '10px', minHeight: '100%' }}>
                    <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
                      ...
                      />
                      <Button variant="contained" size="small" onClick={handleRefreshSearch} sx={{ whiteSpace: 'nowrap', minWidth: 'auto', px: 2 }}>Refresh</Button>
                    </Box>
