...
function ShopPosUpdateForm() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`${ENDPOINT_URL}/get-pos/${id}`)
        setCustomerName(res.data.data.customerName);
        setFactureNumber(Number(res.data?.data?.factureNumber || res.data?.factureNumber || 0));
        ...
        const hasTva = res.data.data.CheckTvA !== undefined ? res.data.data.CheckTvA : res.data.data.checkTvA;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(`${ENDPOINT_URL}/update-pos/${id}`, data);
      if (res) {
        // Open Loading View
        handleQty()
