...
function ItemPurchaseUpdateForm() {
  const { id } = useParams();
  const { isLocked, lockConfig, lockError, forceRelease } = useDocumentLock(id, 'itemPurchase');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(`${ENDPOINT_URL}/update-itemPurchase/${id}`, data, lockConfig)
      if (res) {
        // Open Loading View
        //Update Item Qty
        //handleUpdateQty();
