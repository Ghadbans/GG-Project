...
function PayRollReportInfo({ onMonth, onPayRoll, selectedYear }) {
  ...
  filterMonthPayRoll.forEach(row => {
    monthArray.push({
      ...
      description: 'Net Payable For ' + row.employeeName.name,
  ...
  let amount3 = 0;
  const PayRoll = monthArray?.sort((a, b) => new Date(a.date) - new Date(b.date)).map((row) => {
  ...
  const exportToExcel = async () => {
    ...
    workSheet.columns = [
      ...
      {
        header: "Total PayRoll", key: 'balance', width: 20
      },
    ];
    ...
    return (
      <tr key={i}>
        ...
        <td style={{ textAlign: 'left', borderBottom: '1px solid #DDD' }}>
          <span>{row.type === 'Pay Slip' && (row.description + ' REf PAY-' + row.number)}</span>
          <span>{row.type === 'Expenses' && ('Ref D-' + row.number + ' / ' + row.reason + ' / ' + row.description)}</span>
          <span>{row.type === 'Item Purchase' && ('Ref IP-' + row.number + ' / ' + row.reason + ' / ' + row.description)}</span>
      ...
      <section style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <FormControl sx={{ width: '200px' }}>
          ...
          <Select
            id="statusInfos"
            value={statusInfos}
            onChange={(e) => setStatusInfos(e.target.value)}
            name="statusInfos"
            label="select Status"
          >
                  ...
                  <div className='content' style={{ marginBottom: '20px', position: 'relative' }}>
                    <section style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                      ...
                      <table className="firstTable" style={{ position: 'relative', fontSize: '70%', left: '83px', marginBottom: '10px', pageBreakInside: 'auto' }}>
                        ...
                        <tbody>
                          <tr>
                            <td colSpan={2} style={{ backgroundColor: '#e8f7fe', border: 'none', textAlign: 'left' }}>PayRoll Summary</td>
                          </tr>
                          <tr>
...
export default PayRollReportInfo
