...
function PaymentReportInf({ onMonth, onPayment, selectedYear }) {
  ...
  useEffect(() => {
    const TPayment = filterMonthPayment.length > 0 ? filterMonthPayment.reduce((sum, row) => {
      ...
      return row.transactionType === 'Refund' ? sum - row.amount : sum + row.amount;
    }, 0) : 0
  ...
  const Payment = monthArray?.sort((a, b) => new Date(a.date) - new Date(b.date)).map((row) => {
    if (row.type === 'Payment') {
      if (row.status !== 'Voided') {
        amount3 += (row.transactionType === 'Refund' ? -parseFloat(row.payment) : parseFloat(row.payment))
      }
    ...
    return ({
      ...
      Details: row.type === 'Payment' && row.numberArray.length === 0 && row.credit > 0 ? (`PAY-00${row.number} $${row.credit} In Advanced Payment (Credit) `) : (`PAY-00${row.number} $${Math.abs(parseFloat(row.payment))} for ${row.transactionType === 'Refund' ? 'refund' : 'payment'} of ${row.numberArray?.map((row2) => `INV-00${row2.Ref}`)} / Mode: ${row.defect}${row.status === 'Voided' ? ' [VOIDED]' : ''}`),
      balance: `$${amount3.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}`,
      payment: row.type === 'Payment' ? `$${(row.transactionType === 'Refund' ? -row.payment : row.payment).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}` : '',
    })
  }
  )
  ...
  const allStandingRow = monthArray?.sort((a, b) => new Date(a.date) - new Date(b.date)).map((row, i) => {
    if (row.type === 'Payment') {
      if (row.status !== 'Voided') {
        amount2 += (row.transactionType === 'Refund' ? -parseFloat(row.payment) : parseFloat(row.payment))
      }
    } else if (row.type !== 'Payment') {
    ...
    return (
      <tr key={i} style={{ opacity: row.status === 'Voided' ? 0.5 : 1, textDecoration: row.status === 'Voided' ? 'line-through' : 'none' }}>
        ...
        <td style={{ textAlign: 'left', borderBottom: '1px solid #DDD' }}>
          <span>{row.type === 'Pay Slip' && (`${row.description} Ref PAY-0${row.number}`)}</span>
          <span>{row.type === 'Expenses' && (`Ref D-0${row.number} / ${row.reason} / ${row.description}`)}</span>
          <span>{row.type === 'Item Purchase' && (`Ref ${row.number} / ${row.reason} / ${row.description}`)}</span>
          <span>{row.type === 'Payment' && row.numberArray.length === 0 && row.credit > 0 && (`PAY-00${row.number} $${row.credit} In Advanced Payment (Credit) `)}</span>
          <span>{row.type === 'Payment' && row.numberArray.length > 0 && (`PAY-00${row.number} $${Math.abs(parseFloat(row.payment))} for ${row.transactionType === 'Refund' ? 'refund' : 'payment'} of ${row.numberArray?.map((row2) => `INV-00${row2.Ref}`)} / Mode: ${row.defect}`)}</span>
