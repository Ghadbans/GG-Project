...
function ItemPurchaseReportInfo({onMonth,onItemPurChase}) {
    ...
    filterMonthItemPurchase.forEach(row=>{
      ...
      let amount3 = 0;
      const ItemPurchase = monthArray?.sort((a,b)=> new Date(a.date) - new Date(b.date)).map((row)=>{
        if (row.type === 'Item Purchase') {
      ...
      const exportToExcel = async () => {
        ...
        workSheet.columns = [
          ...
          {
            header:"Total Purchase", key:'balance',width: 20
          },
        ];
...
<div className='content' style={{marginBottom:'20px',position:'relative'}}>
  ...
  <address style={{position:'relative',lineHeight:1.35,width:'60%'}}>
            ...
            <table className="firstTable" style={{position:'relative',fontSize:'70%',left:'83px',marginBottom:'10px',pageBreakInside:'auto'}}>
              ...
              <tbody>  
                <tr>
                  <td colSpan={2} style={{backgroundColor:'#e8f7fe',border:'none',textAlign:'left'}}>Item Purchase Summary</td>
                  </tr>    
                <tr>
...
export default ItemPurchaseReportInfo