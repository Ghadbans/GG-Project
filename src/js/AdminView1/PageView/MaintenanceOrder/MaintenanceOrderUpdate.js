...
import AssetControlReportSection, { DEFAULT_ASSET_CONTROL_REPORT } from '../MaintenanceView/AssetControlReportSection';
...
function MaintenanceOrderUpdate() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [resMaintenance, resItem, resCustomer, resEmployee, resRate] = await Promise.all([
        ...
        const mData = resMaintenance.data.data;
        if (mData) {
  ...
  const handleCreateComment = async () => {
    const data = {
      idInfo: id,
      person: `${user.data.userName} Modify M-${String(serviceNumber).padStart(6, '0')}`,
      reason,
    ...
    return (
      <Box sx={{ width: '100%', minHeight: '100vh', backgroundColor: '#F8FAFC', pb: 12 }}>
        ...
        <Box sx={{ p: 2 }}>
          ...
          {/* Card 6: Parts / Items Used (Technical View - Zero Prices / Zero Costs) */}
...
export default MaintenanceOrderUpdate
