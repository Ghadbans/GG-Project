...
useEffect(()=> {
  const fetchInvoice = async () => {
    if (navigator.onLine) {
      try {
      const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-invoice/${id}`)  
...
if (navigator.onLine) {
      try {
        const res = await axios.get('https://globalgate-backend-production.up.railway.app/endpoint/estimation')
...
if (navigator.onLine) {
  ...
  fetchComment()
   ...
   for (const InvoiceToUpdates of InvoiceToUpdate) {
    try{
      const res = await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-invoice/${InvoiceToUpdates._id}`,{
...
{/** Payment End */}
  ...
  const handleSubmitUpdateStatus =  async (e) => {
    ...
    try {
      const res = await  axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-invoice/${id}`,data)
      if (res) {
        setUpdate('true')
