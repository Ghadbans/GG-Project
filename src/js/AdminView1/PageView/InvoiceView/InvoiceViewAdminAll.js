...
function InvoiceViewAdminAll() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (
          (invoiceData.status === 'Partially-Paid' || invoiceData.status === 'Sent' ||
           invoiceData.status === 'Draft' || invoiceData.status === 'Pending') &&
          parseFloat(invoiceData.balanceDue) <= 0 &&
          parseFloat(invoiceData.totalInvoice) > 0
        ) {
          try { await axios.put(`${ENDPOINT_URL}/update-invoice/${invoiceData._id}`, { status: 'Paid' }); } catch (err) {}
        }

        setInvoiceInfo(
          invoices.filter((row) => row.status === 'Sent' || row.status === 'Partially-Paid')
            .map((row) => ({
              id: row._id,
        ...
        const [resItem, resItemOut, resIReturn] = await Promise.all([
          cachedGet(`${ENDPOINT_URL}/item`),
          cachedGet(`${ENDPOINT_URL}/itemOut`),
          cachedGet(`${ENDPOINT_URL}/itemReturn`),
        ]);
        SetItems(resItem.data.data);
        setItemOut(resItemOut.data?.data?.filter((row) => row.reference._id === id).map((row) => ({ ...row, outNumber: "O-" + String(row.outNumber).padStart(6, '0'), type: 'Item Out' })));
        setItemReturn(resIReturn.data?.data?.filter((row) => row.reference._id === id).map((row) => ({ ...row, outNumber: "R-" + String(row.outNumber).padStart(6, '0'), type: 'Item return' })));
        await cachedGet(`${ENDPOINT_URL}/expensesCategory`);
      } catch (error) {
        console.error('Error fetching data:', error);
        setLoadingData(false);
      }
  ...
  const handleSynced = async (e) => {
    ...
    const updatePurchase = relatedPurchase.map((row) => {
      return axios.put(`${ENDPOINT_URL}/update-invoice/${row._id}`, {
        items: row.items
      })
    })
    try {
    ...
    const fetchData = async () => {
      try {
        const [estimateResponse, purchaseResponse, maintenanceResponse] = await Promise.all([
          cachedGet(`${ENDPOINT_URL}/estimation?summary=true`),
  ...
  useEffect(() => {
    const fetchInvoice2 = async () => {
      if (purchase.length > 0) {
        try {
          const res = await cachedGet(`${ENDPOINT_URL}/estimation?summary=true`)
  ...
  useEffect(() => {
    const fetchComment = async () => {
      try {
        const res = await cachedGet(`${ENDPOINT_URL}/comment`)
        ...
        const resPur = await cachedGet(`${ENDPOINT_URL}/purchase`)
  ...
  useEffect(() => {
    const fetchlastNumber = async () => {
      try {
        ...
        setPaymentNumber((parseInt(res.data?.data?.paymentNumber || res.data?.paymentNumber || 0)) + 1)
      } catch (error) {
        console.error('Error fetching data:', error);
      }
    }
  ...
  const handleUpdateInvoice = async () => {
    const invoiceUpdate = invoiceInfo.length > 0 ? invoiceInfo.map((row) => {
      ...
      const difference = Math.round((correspondingId ? correspondingId.balanceDue - row.total : row.balanceDue) * 100) / 100;
  ...
  const handleSubmitUpdateStatus = async (e) => {
    ...
    try {
      const res = await axios.put(`${ENDPOINT_URL}/update-invoice/${id}`, data)
      if (res) {
        setUpdate('true')
  ...
  const exportToExcel = async () => {
    ...
    saveAs(bold, `${'INV-' + String(invoiceNumber).padStart(6, '0') + ' for ' + customerName1}.xlsx`)
  }
  return (
    <div>
      <Box sx={{ display: 'flex' }}>
                                      ...
                                      <MenuItem>
                                        ...
                                        }
                                        {
                                          row.invoicePurchase === 'Make' && (
                                            <NavLink to={`/MakePurchase/${row._id}`} className='LinkName' style={{ color: 'gray' }}>
                                              <span>Convert To Purchase (Again)</span>
                                            </NavLink>
                                          )
                                        }
                                        {
                                                  ...
                                                  <section style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', boxSizing: 'border-box', padding: '0px', marginBottom: '10px' }}>
                                                    ...
                                                    <div style={{ marginLeft: 'auto', marginRight: config['--metadata-block-right-offset'] || '0px', width: 'fit-content', display: 'block', marginBottom: '10px' }}>
                                                      <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', columnGap: '24px', rowGap: '6px', fontSize: '70%', alignContent: 'start', position: 'relative' }}>
                                                        ...
                                                        <div style={{ textAlign: 'left' }}>INV-{String(row.invoiceNumber).padStart(6, '0')}</div>
                                                      
                                                        <div style={{ fontWeight: 'bold', textAlign: 'left' }}>Date</div>
                                                        <div style={{ textAlign: 'left' }}>{dayjs(row.invoiceDate).format('DD/MM/YYYY')}</div>
                                                        
                                                        <div style={{ fontWeight: 'bold', textAlign: 'left' }}>Due Date</div>
                                                        <div style={{ textAlign: 'left' }}>{dayjs(row.invoiceDueDate).format('DD/MM/YYYY')}</div>
                                                      
                                                      {row.invoiceSubject !== undefined && row.invoiceSubject !== "" && (
                                                        <>
                                                          <div style={{ fontWeight: 'bold', textAlign: 'left' }}>Subject</div>
                                                          <div style={{ textAlign: 'left', wordBreak: 'break-word', maxWidth: '400px' }}>{row.invoiceSubject.toUpperCase()}</div>
                                                        </>
                                                      )}
                                                      
                                                      {row.invoiceDefect !== undefined && row.invoiceDefect !== "" && (
                                                        <>
                                                          <div style={{ fontWeight: 'bold', textAlign: 'left' }}>Defect</div>
                                                          <div style={{ textAlign: 'left', wordBreak: 'break-word', maxWidth: '400px' }}>{row.invoiceDefect.toUpperCase()}</div>
                                                        </>
                                                      )}
                                                          ...
                                                          row.CheckTvA ?
                                                            (
                                                              <tr>
                                                                ...
                                                                <td style={{ textAlign: 'right', borderBottom: '1px solid #DDD' }} colSpan={2}><span data-prefix>$</span><span>{row.tax?.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</span></td>
                                                              </tr>
                                                            )
                                                            : ''
                                                        }
