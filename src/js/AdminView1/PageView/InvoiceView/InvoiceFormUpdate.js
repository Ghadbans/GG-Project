...
function InvoiceFormUpdate() {
  ...
  const fetchItem = async () => {
    if (navigator.onLine) {
      try {
        ...
        const resC = await axios.get('https://globalgate-backend-production.up.railway.app/endpoint/customer')
  ...
  const handleAddToInvoiceUpdate = (shopItem) => {
        ...
        <Box sx={{ ...style, width: 500 }}>
          ...
          <Grid container sx={{ alignItems: 'center', padding: '15px' }} spacing={2}>
            <Grid item xs={12} sx={{ textAlign: 'center' }}>
              <Typography>Do you want to stop Updating invoice ? </Typography>
