    ...
    const [inputValue, setInputValue] = React.useState('');
      ...
      useEffect(()=>{
        const handleFetch = async () => {
          try {
            ...
            const resItemOut = await axios.get('https://globalgate-backend-production.up.railway.app/endpoint/get-last-saved-itemPurchase')
      ...
      useEffect (() => {
        const fetchData = async () => {
          try {
         const res = await axios.get(`https://globalgate-backend-production.up.railway.app/endpoint/get-purchaseOrder/${id}`)
      ...
      useEffect(()=>{
          const fetchPur = async () => {
            ...
            const resM = await axios.get('https://globalgate-backend-production.up.railway.app/endpoint/maintenance')
            setMaintenanceInfo(resM.data.data.filter((row)=>row._id === projectName._id));
          } else if (reason === 'Invoice') {
            const resM = await axios.get('https://globalgate-backend-production.up.railway.app/endpoint/invoice')
...
if ( updateProjectPurchase !== null && updateMaintenance === null&& updateInvoice === null) {
  const updateRequestInvoice = updateProjectPurchase.map((row)=>{
    return axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-purchase/${row.id}`,{
      items:row.items,
  ...
  const updateRequestMaintenance = updateMaintenance.map((row)=>{
    return axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-maintenance/${row.id}`,{
      items:row.items,
    })
  }) 
    try {
  ...
  const updateRequestInvoice = updateInvoice.map((row)=>{
    return axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-invoice/${row.id}`,{
      items:row.items,
    })
  }) 
    try {
...
const onStatusUpdate = async (ReferenceInfo,ReferenceInfoNumber) => {
 ...
 await axios.put(`https://globalgate-backend-production.up.railway.app/endpoint/update-purchaseOrder/${id}`,data);
  } catch (error) {
    console.log(error)
  }
}
...
const [saving,setSaving] = useState('')
  ...
  const handleSubmit = async (e)=>{
    ...
    try{
      const res = await axios.post('https://globalgate-backend-production.up.railway.app/endpoint/create-itemPurchase',data);
      if (res) {
        // Open Loading View
        const ReferenceInfo = res.data.data._id
        const ReferenceInfoNumber = res.data.data.itemPurchaseNumber
        onStatusUpdate(ReferenceInfo,ReferenceInfoNumber)
