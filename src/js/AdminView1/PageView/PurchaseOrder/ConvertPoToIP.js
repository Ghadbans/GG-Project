  ...
  useEffect(() => {
    const handleFetch = async () => {
      try {
        ...
        setItemPurchaseNumber((parseInt(resItemOut.data?.data?.itemPurchaseNumber || resItemOut.data?.itemPurchaseNumber || 0)) + 1)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setManufacturerNumber(Number(res.data?.data?.manufacturerNumber || res.data?.manufacturerNumber || 0));
        setProjectName({
...
const arrayItemRelated = items.filter((row) => row.itemQty !== 0 || row.newDescription !== undefined)

  const handleUpdatePurchase = async () => {
    if (arrayItemRelated.length === 0 || !projectName?._id) return;
  ...
  const onStatusUpdate = async (ReferenceInfo, ReferenceInfoNumber) => {
    try {
      // Fetch latest PO record to avoid overwriting concurrent changes
      const currentPORes = await axios.get(`${ENDPOINT_URL}/get-purchaseOrder/${id}`);
      const latestPOData = currentPORes.data.data;

      // Logic to merge current form items with original PO items to avoid data loss
      const mergedItems = latestPOData.itemsQtyArray.map(originalItem => {
        const updatedItem = items.find(i => i.idRow === originalItem.idRow);
        if (updatedItem) {
          return {
            ...originalItem,
      ...
      const existingIds = new Set(latestPOData.itemsQtyArray.map(i => i.idRow));
      const addedItems = arrayItemRelated.filter(i => !existingIds.has(i.idRow)).map(newItem => ({
        ...newItem,
        itemQty: newItem.itemQty,
        qtyBuy: newItem.itemQty
      }));

      const finalItems = [...mergedItems, ...addedItems];
      ...
      await axios.put(`${ENDPOINT_URL}/update-purchaseOrder/${id}`, data);
    } catch (error) {
      console.log(error)
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    const data = {
      ...
      itemPurchaseDate,
      itemPurchaseNumber,
      manufacturer, manufacturerID, status: "UnPaid",
      manufacturerNumber,
    ...
    try {
      ...
      if (res) {
        ...
        await onStatusUpdate(ReferenceInfo, ReferenceInfoNumber)
            ...
            Item.newDescription !== undefined ? (
              <div style={{ display: 'flex', gap: '30px', alignItems: 'center' }}>
                <TextField
                  ...
                  sx={{ width: '300px', backgroundColor: 'white', fontWeight: 'bold' }}
                />
              ...
              <div style={{ display: 'flex', gap: '30px', alignItems: 'center' }}>
                <div>
                  <Typography sx={{ fontSize: '16px', fontWeight: 'bold' }}>{Item.itemName.itemName}</Typography>
                  <TextField
                ...
                <BlackTooltip title="Clear" placement='top'>
                  <IconButton onClick={() => handleShowAutocomplete(Item.idRow)}>
                    <RemoveCircleOutline style={{ color: '#202a5a' }} />
                  </IconButton>
                </BlackTooltip>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center' }}>
                <Autocomplete
                  ...
                  renderInput={(params) => <TextField multiline rows={4} {...params} required />}
                  ...
                  PaperComponent={({ children, ...other }) => (
                    <Box {...other} sx={{ backgroundColor: 'white', zIndex: 10 }}>
                      {children}
      ...
      >
        <Box sx={{ ...style, width: 500 }}>
          ...
          <Grid container sx={{ alignItems: 'center', padding: '15px' }} spacing={2}>
            ...
            <br />
            <Grid item xs={6}>
              <button type='submit' onClick={() => navigate('/PurchaseOrderViewAdmin')} className='btnCustomer' style={{ width: '100%' }}>Yes</button>
            </Grid>
            <Grid item xs={6}>
          ...
          {loading ? (<Loader />
            ...
            <div style={{ justifyContent: 'center', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '60px', justifyContent: 'center' }}>
                <button onClick={() => navigate('/PurchaseOrderViewAdmin')} className='btnCustomer'>
                  Go Back
                </button>
              </div>
            </div>
