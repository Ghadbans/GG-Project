...
function PurchaseUpdateOrder() {
  const { id } = useParams();
  const { isLocked, lockConfig, lockError, forceRelease } = useDocumentLock(id, 'purchaseOrder');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(`${ENDPOINT_URL}/update-purchaseOrder/${id}`, data, lockConfig);
      if (res) {
        // Open Loading View
        handleCreateComment()
        handleOpen();
