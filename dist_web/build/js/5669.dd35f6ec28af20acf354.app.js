...
function getCustomerIdentity(record) {
  var customersList = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
  if (!record) return {
    key: 'unknown',
    id: 'unknown',
    name: 'Unknown Customer'
  };
  var raw = record.customerName || record.Customer || record.customer;
  var id = null;
  var name = null;
  ...
  if (customersList && customersList.length > 0) {
    if (id) {
      var found = customersList.find(c => String(c._id) === String(id));
      if (found) {
        name = found.Customer || found.customerName || name;
      }
    }
    if (name && !id) {
      var _found = customersList.find(c => (c.Customer || c.customerName || '').trim().toLowerCase() === name.trim().toLowerCase());
      if (_found) {
        id = _found._id;
        name = _found.Customer || _found.customerName || name;
      }
    }
  }

  // Unified key: if ID exists use ID, else normalize lower-case name
  var normalizedKey = id ? String(id) : name ? name.trim().toLowerCase() : 'unknown';
  return {
    key: normalizedKey,
    id: id || normalizedKey,
...
function SalesByCustomerReport(_ref) {
  var onInvoice = _ref.onInvoice,
    ...
    _ref$customers = _ref.customers,
    customers = _ref$customers === void 0 ? [] : _ref$customers;
  ...
  var processedData = (0,react.useMemo)(() => {
    ...
    if (onInvoice && Array.isArray(onInvoice)) {
      allTransactions = [...allTransactions, ...onInvoice.filter(inv => inv.status && !['Draft', 'Decline', 'Void'].includes(inv.status)).map(inv => _objectSpread(_objectSpread({}, inv), {}, {
        ...
        paid: 0,
        // Invoices create receivable debits; payments credit them
    ...
    if (onPos && Array.isArray(onPos)) {
      allTransactions = [...allTransactions, ...onPos.filter(p => p.customerName && (p.customerName.Customer || p.customerName.customerName || typeof p.customerName === 'string')).map(pos => {
        var posTotal = (pos.TotalAmountPaid || pos.totalInvoice || 0) / (pos.rate || 1);
        return _objectSpread(_objectSpread({}, pos), {}, {
          type: 'POS',
          date: pos.invoiceDate || pos.paymentDate,
          amount: posTotal,
          paid: posTotal,
          // POS is fully paid tender
          due: 0
        });
      })];
    }
    ...
    if (onPayment && Array.isArray(onPayment)) {
      allTransactions = [...allTransactions, ...onPayment.filter(pay => pay.status !== 'Voided').map(pay => {
        var pAmount = parseFloat(pay.amount || 0);
        if (pay.modes === 'Credit') {
          var usdPart = parseFloat(pay.PaymentReceivedUSD || 0);
          var fcPart = parseFloat(pay.PaymentReceivedFC || 0) / parseFloat(pay.rate || 1);
        ...
        var rem = parseFloat(pay.remaining || 0);
        var appliedAmount = Math.max(0, pAmount - rem);
    ...
    filteredTransactions.forEach(item => {
      var custInfo = getCustomerIdentity(item, customers);
      var id = custInfo.key;
    ...
    Object.values(customerMap).forEach(c => {
      ...
      var custPayments = c.transactions.filter(t => t.type === 'Payment');
      custPayments.forEach(p => {
        var remVal = p.remaining !== undefined && p.remaining !== null && p.remaining !== '' ? parseFloat(p.remaining) : parseFloat(p.rawAmount || p.amount || 0);
      ...
      var rawBalance = c.totalSales - c.totalPaid - customerCredit;
      // Snap floating-point rounding dust (-0.05 to 0.05) to 0
      c.balance = Math.abs(rawBalance) < 0.05 ? 0 : rawBalance;
    });
    var searchTokens = searchTerm.trim().toLowerCase().split(/\s+/).filter(Boolean);
    return Object.values(customerMap).filter(c => {
      if (!searchTokens.length) return true;
      var normName = (c.name || '').replace(/\s+/g, ' ').toLowerCase();
  ...
  return /*#__PURE__*/react.createElement(Box/* default */.A, null, /*#__PURE__*/react.createElement(Box/* default */.A, {
  ...
  }, "$", ((selectedCustomer === null || selectedCustomer === void 0 ? void 0 : selectedCustomer.totalSales) || 0).toLocaleString(undefined, {
  ...
  }, "$", ((selectedCustomer === null || selectedCustomer === void 0 ? void 0 : selectedCustomer.totalPaid) || 0).toLocaleString(undefined, {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2
  }))), /*#__PURE__*/react.createElement(Grid/* default */.Ay, {
    item: true,
    ...
    variant: "overline",
    color: "textSecondary"
  }, "Available Credit"), /*#__PURE__*/react.createElement(Typography/* default */.A, {
    variant: "h5",
    sx: {
      color: '#ed6c02',
      fontWeight: 'bold'
    }
  }, "-$", ((selectedCustomer === null || selectedCustomer === void 0 ? void 0 : selectedCustomer.availableCredit) || 0).toLocaleString(undefined, {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2
  }))), /*#__PURE__*/react.createElement(Grid/* default */.Ay, {
    item: true,
    ...
    variant: "h5",
    sx: {
      color: ((selectedCustomer === null || selectedCustomer === void 0 ? void 0 : selectedCustomer.balance) || 0) > 0 ? '#d32f2f' : '#2e7d32',
      fontWeight: 'bold'
    }
  }, "$", ((selectedCustomer === null || selectedCustomer === void 0 ? void 0 : selectedCustomer.balance) || 0).toLocaleString(undefined, {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2
    ...
    return selectedCustomer === null || selectedCustomer === void 0 || (_selectedCustomer$tra = selectedCustomer.transactions) === null || _selectedCustomer$tra === void 0 ? void 0 : _selectedCustomer$tra.sort((a, b) => new Date(a.date) - new Date(b.date)).map((item, idx) => {
      ...
      var displayBalance = Math.abs(runningBalance) < 0.05 ? 0 : runningBalance;
      return /*#__PURE__*/react.createElement(TableRow/* default */.A, {
      ...
      }, item.type)), /*#__PURE__*/react.createElement(TableCell/* default */.A, null, item.type === 'Invoice' ? "INV-".concat(String(item.invoiceNumber || '').padStart(6, '0')) : item.type === 'POS' ? "POS-".concat(String(item.factureNumber || '').padStart(6, '0')) : "PAY-".concat(String(item.paymentNumber || '').padStart(6, '0'))), /*#__PURE__*/react.createElement(TableCell/* default */.A, {
        align: "right"
        ...
        align: "right"
      }, "$", (item.paid || 0).toLocaleString(undefined, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2
      })), /*#__PURE__*/react.createElement(TableCell/* default */.A, {
        align: "right",
...
function ARAgingReport_getCustomerIdentity(record) {
  var customersList = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
  if (!record) return {
    key: 'unknown',
    id: 'unknown',
...
function ARAgingReport(_ref) {
  ...
  var customerInvoices = (0,react.useMemo)(() => {
    ...
    return onInvoice.filter(inv => {
      ...
      var custInfo = ARAgingReport_getCustomerIdentity(inv, customers);
  ...
  var agingData = (0,react.useMemo)(() => {
    ...
    onInvoice.forEach(inv => {
      ...
      if (balance <= 0) return;
      var custInfo = ARAgingReport_getCustomerIdentity(inv, customers);
      var custId = custInfo.key;
      var custName = custInfo.name;
...
function SupplierReportInfo(_ref) {
  ...
  var processedData = (0,react.useMemo)(() => {
    ...
    return Object.values(supplierMap).filter(s => {
      var normName = (s.name || '').replace(/\s+/g, ' ').toLowerCase();
      var normStore = (s.storeName || '').replace(/\s+/g, ' ').toLowerCase();
      var matchesSearch = !searchTokens.length || searchTokens.every(token => normName.includes(token) || normStore.includes(token));
...
function ReportsViewAdmin() {
  ...
  var _useState13 = (0,react.useState)([]),
    ...
    grantAccess = _useState14[0],
    setGrantAccess = _useState14[1];
  var _useState15 = (0,react.useState)(true),
    _useState16 = ReportsViewAdmin_slicedToArray(_useState15, 2),
    isFetchingAccess = _useState16[0],
    setIsFetchingAccess = _useState16[1];
  (0,react.useEffect)(() => {
    var _user$data;
  ...
  (0,react.useEffect)(() => {
    ...
    var fetchAllData = /*#__PURE__*/function () {
      var _ref7 = ReportsViewAdmin_asyncToGenerator(function* () {
        try {
          var _resItems$data, _resInvoices$data, _resExpenses$data, _resPayroll$data, _resPayments$data, _resItemPurchases$dat, _resMaintenance$data, _resProjects$data, _resPos$data, _resItemOut$data, _resSuppliers$data, _resCustomers$data;
          setLoadingData(true);
          var _yield$Promise$all = yield Promise.all([(0,apiCache/* cachedGet */.Fe)("".concat(apiConfig/* ENDPOINT_URL */.m, "/item")), (0,apiCache/* cachedGet */.Fe)("".concat(apiConfig/* ENDPOINT_URL */.m, "/invoice?summary=true")), (0,apiCache/* cachedGet */.Fe)("".concat(apiConfig/* ENDPOINT_URL */.m, "/expense?summary=true")), (0,apiCache/* cachedGet */.Fe)("".concat(apiConfig/* ENDPOINT_URL */.m, "/payRoll")), (0,apiCache/* cachedGet */.Fe)("".concat(apiConfig/* ENDPOINT_URL */.m, "/payment")), (0,apiCache/* cachedGet */.Fe)("".concat(apiConfig/* ENDPOINT_URL */.m, "/itemPurchase?summary=true")), (0,apiCache/* cachedGet */.Fe)("".concat(apiConfig/* ENDPOINT_URL */.m, "/maintenance?summary=true")), (0,apiCache/* cachedGet */.Fe)("".concat(apiConfig/* ENDPOINT_URL */.m, "/projects")), (0,apiCache/* cachedGet */.Fe)("".concat(apiConfig/* ENDPOINT_URL */.m, "/pos?summary=true")), (0,apiCache/* cachedGet */.Fe)("".concat(apiConfig/* ENDPOINT_URL */.m, "/item-usage")), (0,apiCache/* cachedGet */.Fe)("".concat(apiConfig/* ENDPOINT_URL */.m, "/Supplier")), (0,apiCache/* cachedGet */.Fe)("".concat(apiConfig/* ENDPOINT_URL */.m, "/customer"))]),
            _yield$Promise$all2 = ReportsViewAdmin_slicedToArray(_yield$Promise$all, 12),
            ...
            resCustomers = _yield$Promise$all2[11];
          var fetchedData = {
            items: ((_resItems$data = resItems.data) === null || _resItems$data === void 0 ? void 0 : _resItems$data.data) || [],
            invoices: ((_resInvoices$data = resInvoices.data) === null || _resInvoices$data === void 0 ? void 0 : _resInvoices$data.data) || [],
            expenses: ((_resExpenses$data = resExpenses.data) === null || _resExpenses$data === void 0 ? void 0 : _resExpenses$data.data) || [],
            payroll: ((_resPayroll$data = resPayroll.data) === null || _resPayroll$data === void 0 ? void 0 : _resPayroll$data.data) || [],
            payments: ((_resPayments$data = resPayments.data) === null || _resPayments$data === void 0 ? void 0 : _resPayments$data.data) || [],
            itemPurchases: ((_resItemPurchases$dat = resItemPurchases.data) === null || _resItemPurchases$dat === void 0 ? void 0 : _resItemPurchases$dat.data) || [],
            maintenance: ((_resMaintenance$data = resMaintenance.data) === null || _resMaintenance$data === void 0 ? void 0 : _resMaintenance$data.data) || [],
            projects: ((_resProjects$data = resProjects.data) === null || _resProjects$data === void 0 ? void 0 : _resProjects$data.data) || [],
            pos: ((_resPos$data = resPos.data) === null || _resPos$data === void 0 ? void 0 : _resPos$data.data) || [],
            ...
            customers: ((_resCustomers$data = resCustomers.data) === null || _resCustomers$data === void 0 ? void 0 : _resCustomers$data.data) || []
          };
          setData(fetchedData);
        } catch (error) {
  ...
  return /*#__PURE__*/react.createElement("div", {
    ...
    ref: componentRef
  }, loadingData ? /*#__PURE__*/react.createElement(Box/* default */.A, {
    sx: {
      display: 'flex',
  ...
  }, /*#__PURE__*/react.createElement(CircularProgress/* default */.A, {
    size: 48,
    sx: {
      color: '#30368a'
    }
    ...
    variant: "body2",
    color: "textSecondary"
  }, "Loading report data...")) : /*#__PURE__*/react.createElement(react.Fragment, null, activeReport === 'revenue' && /*#__PURE__*/react.createElement(RevenueExpensesAll/* default */.A, {
    onMonth: "All",
