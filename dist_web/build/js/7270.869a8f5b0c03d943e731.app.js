...
function CustomerInformationView() {
  ...
  var searchTokens = search.trim().toLowerCase().split(/\s+/).filter(Boolean);
  var newArray = searchTokens.length > 0 ? customer.filter(row => {
    var custType = (row.customerType || '').replace(/\s+/g, ' ').toLowerCase();
    var custName = (row.Customer || row.customerName || '').replace(/\s+/g, ' ').toLowerCase();
    var custEmail = (row.customerEmail || '').toLowerCase();
    var custDesc = (row.customerDescription || '').replace(/\s+/g, ' ').toLowerCase();
    return searchTokens.every(token => custType.includes(token) || custName.includes(token) || custEmail.includes(token) || custDesc.includes(token));
  }) : customer;
  {/** search && Tab End */}
  ...
  (0,react.useEffect)(() => {
    if (selectOptions === 'Year') {
      var openingBalanceTotal1 = newStatement.length > 0 ? newStatement.filter(row => row.type === '***Opening Balance***').reduce((sum, row) => sum + parseFloat(row.amount || 0), 0) : 0;
      setOpeningBalanceTotal(openingBalanceTotal1);
      var InvoiceTotal1 = newStatement.length > 0 ? newStatement.filter(row => row.type === 'Invoice' || row.type === 'POS').reduce((sum, row) => sum + parseFloat(row.amount || 0), 0) : 0;
      setInvoiceTotal(InvoiceTotal1);
      var PaymentTotal1 = newStatement.length > 0 ? newStatement.filter(row => row.type === 'Payment' || row.type === 'POS').reduce((sum, row) => sum + parseFloat(row.payment || 0), 0) : 0;
      ...
      var _InvoiceTotal = AllStatement.length > 0 ? AllStatement.filter(row => row.type === 'Invoice' || row.type === 'POS').reduce((sum, row) => sum + parseFloat(row.amount || 0), 0) : 0;
      setInvoiceTotal(_InvoiceTotal);
      var _PaymentTotal = AllStatement.length > 0 ? AllStatement.filter(row => row.type === 'Payment' || row.type === 'POS').reduce((sum, row) => sum + parseFloat(row.payment || 0), 0) : 0;
      ...
      setOpeningBalanceTotal(0);
      var _InvoiceTotal2 = OutStandStatement.length > 0 ? OutStandStatement.filter(row => row.type === 'Invoice' || row.type === 'POS').reduce((sum, row) => sum + parseFloat(row.amount || 0), 0) : 0;
      var InvoiceTotal2 = OutStandStatement.length > 0 ? OutStandStatement.filter(row => row.type === 'Invoice' || row.type === 'POS').reduce((sum, row) => sum + parseFloat(row.balance !== undefined ? row.balance : row.amount - (row.payment || 0)), 0) : 0;
      var InvoiceTotal3 = OutStandStatement.length > 0 ? OutStandStatement.filter(row => row.type === 'Invoice' || row.type === 'POS').reduce((sum, row) => sum + parseFloat(row.paidAmount !== undefined ? row.paidAmount : row.payment || 0), 0) : 0;
      setInvoiceTotal(_InvoiceTotal2);
      setPaymentTotal(InvoiceTotal3);
      setTotalBalance(InvoiceTotal2);
    } else if (selectOptions === 'Custom') {
      setOpeningBalanceTotal(0);
      var _InvoiceTotal3 = customStatement.length > 0 ? customStatement.filter(row => row.type === 'Invoice' || row.type === 'POS').reduce((sum, row) => sum + parseFloat(row.amount || 0), 0) : 0;
      setInvoiceTotal(_InvoiceTotal3);
      var _PaymentTotal2 = customStatement.length > 0 ? customStatement.filter(row => row.type === 'Payment' || row.type === 'POS').reduce((sum, row) => sum + parseFloat(row.payment || 0), 0) : 0;
