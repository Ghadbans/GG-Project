...
function App() {
  useEffect(() => {
    if (navigator.onLine) {
      const socket = io('http://192.168.0.200:8080', { transports: ['websocket', 'polling'] })
      socket.on('newNotification', (newNotification) => {
        ...
        //const message = `${dayjs(newNotification.dateNotification).format('DD/MM/YYYY-HH:mm')+ ' ' + newNotification.person + ': ' + newNotification.reason}`
