...
const ReturnInfoD = grantAccess.filter((row)=> row.moduleName === "Item-Return" && row.access.deleteM === true);
    ...
    const handleLogout = () => {
      ...
      const fetchItems = async (page, searchTerm, filterField, filterValue) => {
        try {
          const res = await axios.get(`http://192.168.0.200:8080/endpoint/itemReturn-Information?page=${page + 1}&limit=${limit}&search=${encodeURIComponent(searchTerm.trim())}&filterField=${encodeURIComponent(filterField.trim())}&filterValue=${encodeURIComponent(filterValue.trim())}`);
      ...
      const fetchAndSaveData = async () => {
        try {
          const res = await axios.get(`http://192.168.0.200:8080/endpoint/itemReturn`);
          await db.itemReturn.clear();
          await db.itemReturn.bulkPut(res.data.data);
   ...
   useEffect(()=>{
    ...
    useEffect(()=>{
      const fetchData2 = async () => {
        ...
        if (navigator.onLine) {
          try {
            const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-itemReturn/${idView}`)
...
useEffect(()=>{
  const fetchId = async () => {
    if (DeleteId !== null) {
      try {
        const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-itemReturn/${DeleteId}`)
...
const handleDeleteUpdate = async (e) => {
  e.preventDefault()
  try {
    const res = await axios.delete(`http://192.168.0.200:8080/endpoint/delete-itemReturn/${DeleteId}`);
    if (res) {
