...
const ProjectInfoD = grantAccess.filter((row)=> row.moduleName === "Project" && row.access.deleteM === true);
  ...
  const fetchData = async () => {
    if (navigator.onLine) {
      try {
        ...
        await db.projectSchema.clear();
        await Promise.all(res.data.data.map( async (item)=>{
         await db.projectSchema.put({...item, synced: true,updateS:true})
        }))
        ...
        const syncOff = async () => {
          if (navigator.onLine) {
           ...
           for(const projectInfo of projectsToSynced){
            try {
              const res = await axios.post('http://192.168.0.200:8080/endpoint/create-projects',projectInfo)
               if (res) {
            ...
            for(const projectInfoUpdate of projectsToSyncedUpdate){
              try {
                 await axios.put(`http://192.168.0.200:8080/endpoint/update-projects/${projectInfoUpdate._id}`,projectInfoUpdate)
 ...
 const [openDeleteAll, setOpenDeleteAll] = useState(false);
        ...
        useEffect(()=>{
          const fetchFunction = async () => {
            const deletePromises = selectedRows.map( async (idToDelete)=> {
              return axios.get(`http://192.168.0.200:8080/endpoint/get-projects/${idToDelete}`)
            })
            try {
  ...
  const handleDeleteMany = async (e) => {
    e.preventDefault()
          const deletePromises = selectedRows.map( async (idToDelete)=> {
            return axios.delete(`http://192.168.0.200:8080/endpoint/delete-projects/${idToDelete}`);
          })
    try {
      const res = await Promise.all(deletePromises);
            if (res) {
  ...
  const handleDelete  = async () => {
    try {
      const deletePromises = selectedRows.map( async (idToDelete)=> {
        await axios.delete(`http://192.168.0.200:8080/endpoint/delete-projects/${idToDelete}`)
      })
