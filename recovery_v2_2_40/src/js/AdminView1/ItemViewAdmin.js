...
function ItemViewAdmin() {
  ...
  const syncData = async () => {
    if (navigator.onLine) {
      try {
        ...
        setItemToLoad(formatDate);
        // Update IndexedDB with new data
        await db.itemSchema.clear();
        await db.itemSchema.bulkPut(formatDate);
      } catch (error) {
  ...
  const fetchItems = async (page, searchTerm, filterField, filterValue) => {
    try {
      const res = await axios.get(`http://192.168.0.200:8080/endpoint/item-Information?page=${page + 1}&limit=${limit}&search=${encodeURIComponent(searchTerm.trim())}&filterField=${encodeURIComponent(filterField.trim())}&filterValue=${encodeURIComponent(filterValue.trim())}`);
  ...
  const handleLoadMargin = async (e) => {
    e.preventDefault();
    try {
      const resLow = await axios.get('http://192.168.0.200:8080/endpoint/low-margin-item');
  ...
  const handleDelete = async () => {
    try {
      const res = await axios.delete(`http://192.168.0.200:8080/endpoint/delete-item/${DeleteId}`);
      if (res) {
        handleOpenModal();
      }
    } catch (error) {
  ...
  useEffect(() => {
    const fetchFunction = async () => {
      const deletePromises = selectedRows.map(async (idToDelete) => {
        return axios.get(`http://192.168.0.200:8080/endpoint/get-item/${idToDelete}`);
      });
      try {
  ...
  const handleDeleteMany = async (e) => {
    e.preventDefault();
    const deletePromises = selectedRows.map(async (idToDelete) => {
      return axios.delete(`http://192.168.0.200:8080/endpoint/delete-item/${idToDelete}`);
    });
    try {
      const res = await Promise.all(deletePromises);
      if (res) {
  ...
  const newArray = useMemo(() => {
    ...
    return search2 !== '' ? lowMargin.filter((Item) =>
      ...
      (Item.itemUpc && Item.itemUpc.itemNumber.toString().includes(lowerSearch2))
    ) : lowMargin;
  }, [lowMargin, search2]);
