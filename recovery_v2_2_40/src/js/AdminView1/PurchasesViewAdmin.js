  ...
  const fetchData = async () => {
    if (navigator.onLine) {
      try {
        ...
        const offLineCustomer1 = await db.purchaseSchemaView.toArray();
        const formatDate2 = offLineCustomer1.map(item=> item._id);
        const newPurchase =  formatDate.filter(inv => !formatDate2.includes(inv._id));
        const newPurchaseIds = newPurchase.map(inv => inv._id);

        
        setNewPurchase(newPurchaseIds);
        setPurchase(formatDate.reverse());
        
        await db.purchaseSchema.clear();

        await Promise.all(res.data.data.map( async (item)=>{
          await db.purchaseSchema.put({...item, synced:true,updateS:true})
         }))

     await db.purchaseSchemaView.bulkPut(formatDate.map((inv,i)=> ({purchaseNumber:inv.purchaseNumber,_id:inv._id})))
        ...
        const syncOff = async () => {
          if (navigator.onLine) {
           ...
           for(const PurchaseInfo of PurchaseToSynced){
            try {
              const res = await axios.post('http://192.168.0.200:8080/endpoint/create-purchase',PurchaseInfo)
               if (res) {
            ...
            for(const PurchaseInfoUpdate of projectsToSyncedUpdate){
              try {
                 await axios.put(`http://192.168.0.200:8080/endpoint/update-purchase/${PurchaseInfoUpdate._id}`,PurchaseInfoUpdate)
...
const handleDelete = async () => {
  try {
    const res = await axios.delete(`http://192.168.0.200:8080/endpoint/delete-purchase/${DeleteId}`);
    if (res) {
      handleOpenModal() 
    }
  } catch (error) {
        ...
        useEffect(()=>{
          const fetchFunction = async () => {
            const deletePromises = selectedRows.map( async (idToDelete)=> {
              return axios.get(`http://192.168.0.200:8080/endpoint/get-purchase/${idToDelete}`)
            })
            try {
...
const handleDeleteMany = async (e) => {
  e.preventDefault()
  const deletePromises = selectedRows.map( async (idToDelete)=> {
    return axios.delete(`http://192.168.0.200:8080/endpoint/delete-purchase/${idToDelete}`)
  })
  try {
    const res = await Promise.all(deletePromises);
            if (res) {
