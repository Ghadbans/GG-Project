...
function PayRollViewAdmin() {
   ...
   const PRollInfoD = grantAccess.filter((row)=> row.moduleName === "Pay-Roll" && row.access.deleteM === true);
    ...
    const fetchData = async () => {
      if (navigator.onLine) {
        try {
          const res = await axios.get('http://192.168.0.200:8080/endpoint/payRoll')
          ...
          await db.payRollSchema.clear();
          await Promise.all(res.data.data.map( async (item)=>{
            await db.payRollSchema.put({...item, synced: true,updateS:true})
           }))
     ...
     const formatDate = offLineCustomer1.map((item)=>({
      ...item,
      id: item._id,
       dateField: dayjs(new Date(item.payDate)).format('DD/MM/YYYY'),
       visit: dayjs(new Date(item.month)).format('DD/MM/YYYY')
     })) 
     setCustomer(formatDate.reverse())
     setLoadingData(false)
      }
    ...
    const syncOff = async () => {
      if (navigator.onLine) {
        ...
        const PayRollToSynced = syncedPayRoll.filter((row)=>row.synced === false).map(({payNumber,payDate, month, daysW, status, daysOpen,Lops,basicSalary,earningSalary,advancedSalary,basicTransport,transportEarning,transportDeduction,foodBasic,
          ...
          employeeName,amountPayUSD,amountPayFC,CreditFC,CreditUSD})=>({payNumber,payDate, month, daysW, status, daysOpen,Lops,basicSalary,earningSalary,advancedSalary,basicTransport,transportEarning,transportDeduction,foodBasic,
            foodEarning,itemLost,foodDeduction,bounceAllowances,bounceAllowancesEarning,other,otherEarning,loan,
            bonus,totalActualSalary,totalActualEarning,totalActualDeduction,totalNet,totalPaid,rate,totalPaidDollars,words,
       ...
       for(const PayRollInfo of PayRollToSynced){
        try {
           const res = await axios.post('http://192.168.0.200:8080/endpoint/create-payRoll',PayRollInfo)
         if (res) {
        ...
        for(const PayRollInfoUpdate of PayRollToSyncedUpdate){
          try {
             await axios.put(`http://192.168.0.200:8080/endpoint/update-payRoll/${PayRollInfoUpdate._id}`,PayRollInfoUpdate)
    ...
    const handleDelete = async () => {
      try {
        const res = await axios.delete(`http://192.168.0.200:8080/endpoint/delete-payRoll/${DeleteId}`);
        if (res) {
          handleOpenModal();
        }
      } catch (error) {
    ...
    useEffect(()=>{
      const fetchFunction = async () => {
        const deletePromises = selectedRows.map( async (idToDelete)=> {
          return axios.get(`http://192.168.0.200:8080/endpoint/get-payRoll/${idToDelete}`)
        })
        try {
    ...
    const handleDeleteMany = async (e) => {
      e.preventDefault()
      const deletePromises = selectedRows.map( async (idToDelete)=> {
        return axios.delete(`http://192.168.0.200:8080/endpoint/delete-payRoll/${idToDelete}`)
      })
      try {
         const res = await Promise.all(deletePromises);
         if (res) {
    ...
    useEffect (() => {
      const fetchId = async () => {
        if (updateId !== null) {
          try {
           const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-payRoll/${updateId}`)
      ...
      const handleSubmitUpdateStatus = async (e) => {
        ...
        try {
          const res = await  axios.put(`http://192.168.0.200:8080/endpoint/update-payRoll/${updateId}`,data)
          if (res) {
            handleOpenLoading();
          }
        } catch (error) {
