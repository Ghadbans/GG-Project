...
const handleCloseModal = () => {
  ...
  const fetchData = async () => {
    if (navigator.onLine) {
        try {
          const res = await axios.get('http://192.168.0.200:8080/endpoint/employeeattendance')
     ...
     const syncOff = async () => {
      if (navigator.onLine) {
       ...
       for(const EmployeeInfo of EmployeeToSynced){
        try {
           await axios.post('http://192.168.0.200:8080/endpoint/create-employeeattendance',EmployeeInfo)
        ...
        for(const EmployeeInfoUpdate of EmployeeToSyncedUpdate){
          try {
             await axios.put(`http://192.168.0.200:8080/endpoint/update-employeeattendance/${EmployeeInfoUpdate._id}`,EmployeeInfoUpdate)
...
useEffect(()=>{
  const fetchData2 = async () => {
   ...
   if (navigator.onLine) {
     try {
         const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-employeeattendance/${idView}`)
