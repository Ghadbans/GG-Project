      ...
      const handleOpenBack = (e) => {
        ...
        useEffect(()=>{
          const fetchData = async () => {
           if (navigator.onLine) {
             try {
               const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-planing/${id}`)
        ...
        if (status === "Full-Day") {
          ...
          setWorkNumber(numberHour)
            ...
            const handleSubmit = async (e) => {
                ...
                if (navigator.onLine) {
                    try{
                      const res = await axios.put(`http://192.168.0.200:8080/endpoint/update-planing/${id}`,data); 
                      if (res) {
