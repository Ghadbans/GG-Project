...
useEffect(()=> {
  const fetchEId = async () => {
    if (navigator.onLine) {
      ...
      const resDE = await axios.get('http://192.168.0.200:8080/endpoint/expense')
      ...
      setPlaning(offLineCustomer1.filter((row)=> row.employeeId === id).map((row)=>({
        ...row,
        totalWorkDay: parseFloat(row.dayPayUSd * row.workNumber).toFixed(2)
      })))
      const offLineCustomer2 = await db.payRollSchema.toArray();
      setPayRoll(offLineCustomer2.filter((row)=> row.employeeName.id === id))
      const offLineCustomer3 = await db.dailyExpenseSchema.toArray();
      setExpenses(offLineCustomer3.filter((row)=> row.accountName === 'Employee' && row.employeeName.find((Item)=>Item.idRow === id))
                                 .map((row)=>({
                                  ...row,
                                  amountFC: row.employeeName.filter((Item)=>Item.idRow === id).reduce((sum,acc)=> sum + parseFloat(acc.amount),0),
                                  amountUSD: row.employeeName.filter((Item)=>Item.idRow === id).reduce((sum,acc)=> sum + parseFloat(acc.total),0),
...
const planingObject = planing?.reduce((acc,item)=>{
...
const planingArray = Object.keys(planingObject).map((row)=> planingObject[row])
  ...
  useEffect(()=> {
   const fetchImages = async () => {
    if (employeeName !== null) {
      try {
        const resp = await axios.get(`http://192.168.0.200:8080/endpoint/get-image/${employeeName}`);
  ...
  {/** Filter VIew End */}
    ...
    const [uploadedImageURL, setUploadedImageURL]= useState('')
        ...
        const handleImageSubmit = async (e)=> {
          ...
          try {
              await axios.post('http://192.168.0.200:8080/endpoint/upload-image', formData,{
              headers: {
        ...
        const handleDeleteImage = async (e)=> {
          ...
          if (images !== null) {
            try {
              const res = await axios.delete(`http://192.168.0.200:8080/endpoint/delete-image/${images._id}`)
              if (res) {
                handleOpenDelete()
              }
            } catch (error) {
