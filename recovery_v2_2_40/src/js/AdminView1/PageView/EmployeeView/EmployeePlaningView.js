...
if (navigator.onLine) {
      try {
        const res = await axios.get('http://192.168.0.200:8080/endpoint/planing')
  ...
  const syncOff = async () => {
    if (navigator.onLine) {
     ...
     for(const Items of ItemToSynChro){
      try {
         const res = await axios.post('http://192.168.0.200:8080/endpoint/create-planing',Items)
        if (res) {
      ...
      for(const ItemsUpdate of ItemToSynChroUpdate){
        try {
           await axios.put(`http://192.168.0.200:8080/endpoint/update-planing/${ItemsUpdate._id}`,ItemsUpdate)
           handleOpenOffline();
        } catch (error) {
          console.log(error)
        }
...
useEffect(()=>{
  const fetchId = async () => {
    if (DeleteId !== null) {
      try {
        const res = await  axios.get(`http://192.168.0.200:8080/endpoint/get-planing/${DeleteId}`)
...
const handleDeleteUpdate = async (e) => {
  e.preventDefault()
  try {
    const res = await axios.delete(`http://192.168.0.200:8080/endpoint/delete-planing/${DeleteId}`);
    if (res) {
