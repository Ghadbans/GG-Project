 ...
 const [nameDelete,setNameDelete] = useState('')
  useEffect(()=>{
    const fetchData2 = async () => {
      ...
      if (DeleteId !== null) {
      try {
          const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-grantAccess/${DeleteId}`)
  ...
  const handleDelete = async (e) => {
    e.preventDefault()
    try {
      const res = await axios.delete(`http://192.168.0.200:8080/endpoint/delete-grantAccess/${DeleteId}`);
      if (res) {
