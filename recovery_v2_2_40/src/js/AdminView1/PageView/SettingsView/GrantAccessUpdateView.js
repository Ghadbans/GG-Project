...
function GrantAccessUpdateView() {
  ...
  useEffect(()=> {
    const fetchData = async () => {
      try {
        const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-grantAccess/${id}`)
...
const handleCreateComment = async () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try{
      const res = await axios.put(`http://192.168.0.200:8080/endpoint/update-grantAccess/${id}`,data)
      if (res) {
         // Open Loading View
         handleCreateComment()
        handleOpen();
