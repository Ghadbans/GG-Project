...
function ItemReturnViewForm() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-itemReturn/${id}`)
  ...
  useEffect(() => {
    const handleFetch = async () => {
      try {
        ...
        const resProject = await axios.get('http://192.168.0.200:8080/endpoint/purchase')
  ...
  const handleAddToItemReturnUpdate = (shopItem) => {
      ...
      try {
        ...
        res.forEach((resp, index) => {
          QtyUpdate[`itemDb${index + 1}`] =
          {
            ids: resp.data.data._id,
            data: {
              itemQuantity: resp.data.data.itemQuantity + parseFloat(initialStateId[`item${index + 1}`].itemDbQty)
            }
          }
        })
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(`http://192.168.0.200:8080/endpoint/update-itemReturn/${id}`, data)
      if (res) {
        // Open Loading View
        //handleUpdateQty();
        handleUpdateInfo();
...
export default ItemReturnViewForm
