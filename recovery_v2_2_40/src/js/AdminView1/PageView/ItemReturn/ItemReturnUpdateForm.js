...
function ItemReturnUpdateForm() {
  ...
  useEffect(() => {
    const handleFetch = async () => {
      try {
        const resItemOut = await axios.get('http://192.168.0.200:8080/endpoint/get-last-saved-itemReturn')
        ...
        const resProject = await axios.get('http://192.168.0.200:8080/endpoint/projects')
        setProject(resProject.data.data);
        const resEmployee = await axios.get('http://192.168.0.200:8080/endpoint/employee')
  ...
  const handleAddToItemReturn = (shopItem) => {
  ...
  useEffect(() => {
    const fetchId = async () => {
      if (projectName !== null) {
        try {
          const res = await axios.get('http://192.168.0.200:8080/endpoint/purchase')
        ...
        try {
          const res = await axios.get('http://192.168.0.200:8080/endpoint/maintenance')
        ...
        try {
          const res = await axios.get('http://192.168.0.200:8080/endpoint/invoice')
  ...
  const handleUpdateQty = async () => {
    ...
    try {
      ...
      res.forEach((resp, index) => { QtyUpdate[`id${index + 1}`] = { ids: resp.data.data._id, data: { itemQuantity: parseFloat(resp.data.data.itemQuantity) + parseFloat(QtyNew[index]) } } })
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('http://192.168.0.200:8080/endpoint/create-itemReturn', data);
      if (res) {
        // Open Loading View
  ...
  const tableRows = reason !== 'Dotation' ? newArray2.map((Item, i) => {
    ...
    return (
      <tr key={Item.idRow}>
        {
          related.newDescription !== undefined ?
            (
              <>
            ...
            Item.itemName.itemName ? (
              (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div >
                    <Typography sx={{ fontSize: '23px', fontWeight: 'bold' }}>{Item.itemName.itemName}</Typography>
                    <TextField
                      name='itemDescription' id='itemDescription'
      ...
      >
        <Box sx={{ ...style, width: 500 }}>
          ...
          <Grid container sx={{ alignItems: 'center', padding: '15px' }} spacing={2}>
            ...
            <br />
            <Grid item xs={6}>
              <button type='submit' onClick={() => navigate('/ItemReturnAdminView')} className='btnCustomer' style={{ width: '100%' }}>Yes</button>
            </Grid>
            <Grid item xs={6}>
...
export default ItemReturnUpdateForm
