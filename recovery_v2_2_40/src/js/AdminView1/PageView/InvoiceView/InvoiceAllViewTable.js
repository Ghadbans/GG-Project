...
useEffect(()=> {
axios.get('http://192.168.0.200:8080/endpoint/dailyexpense')
.then(res => {
  // Handle the response data here
...
{/** Delete Function */}
  const handleDelete = async (id) => {
    ...
    try {
      const res = await axios.delete(`http://192.168.0.200:8080/endpoint/delete-dailyexpense/${RefId}`);
      alert(res.data.msg); // This will be the deleted document
    } catch (error) {
      console.error(error);
    }
  try {
    const res = await axios.delete(`http://192.168.0.200:8080/endpoint/delete-invoice/${id}`);
    alert(res.data.msg); // This will be the deleted document
  } catch (error) {
    console.error(error);
  }
