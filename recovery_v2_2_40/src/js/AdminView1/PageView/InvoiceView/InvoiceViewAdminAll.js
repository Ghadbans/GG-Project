...
const [itemReturn, setItemReturn] = useState([]);
  const apiUrl = 'http://192.168.0.200:8080/endpoint/invoice';
  useEffect(()=>{
  ...
  const fetchData = async ()=> {
   if (navigator.onLine) {
      ...
      const resItemOut = await axios.get('http://192.168.0.200:8080/endpoint/itemOut')
      setItemOut(resItemOut.data.data.filter((row)=> row.reference._id === id ).map((row)=>({...row, outNumber: "O-0" + row.outNumber,type:'Item Out' })))
      const resIReturn = await axios.get('http://192.168.0.200:8080/endpoint/itemReturn')
...
const handleSynced = async(e)=> {
  ...
  const updatePurchase = relatedPurchase.map((row)=>{
    return axios.put(`http://192.168.0.200:8080/endpoint/update-invoice/${row._id}`,{
      items:row.items
    })  
   }) 
   try {
...
useEffect(()=> {
  const fetchInvoice = async () => {
    if (navigator.onLine) {
      try {
      const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-invoice/${id}`)  
...
useEffect(()=>{
  const fetchCustomer = async () =>{
    ...
    if (customerID !== null){
       if (navigator.onLine) {
        try {
          const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-customer/${customerID}`)
    ...
    if (navigator.onLine) {
      try {
        const [estimateResponse,purchaseResponse,maintenanceResponse] = await Promise.all([
          axios.get('http://192.168.0.200:8080/endpoint/estimation'),
...
if (navigator.onLine) {
  ...
  fetchComment()
   ...
   for (const InvoiceToUpdates of InvoiceToUpdate) {
    try{
      const res = await axios.put(`http://192.168.0.200:8080/endpoint/update-invoice/${InvoiceToUpdates._id}`,{
...
useEffect(()=>{
  const fetchlastNumber = async () => {
   if (navigator.onLine) {
     try {
       const res = await axios.get('http://192.168.0.200:8080/endpoint/get-last-saved-payment')
...
{/** Payment End */}
  ...
  const handleUpdateCredit = async () => {
    ...
    if (navigator.onLine) {
      try {
       await axios.put(`http://192.168.0.200:8080/endpoint/update-customer/${customerID}`,data) 
  ...
  const handleSubmitUpdateStatus =  async (e) => {
    ...
    try {
      const res = await  axios.put(`http://192.168.0.200:8080/endpoint/update-invoice/${id}`,data)
      if (res) {
        setUpdate('true')
  ...
  const handleSubmit = async (e) => {
   ...
   if (navigator.onLine) {
     try{
       const res = await axios.post('http://192.168.0.200:8080/endpoint/create-payment',data); 
       if (res) {
