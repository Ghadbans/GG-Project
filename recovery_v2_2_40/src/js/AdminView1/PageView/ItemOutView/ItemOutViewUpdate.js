...
function ItemOutViewUpdate() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-itemOut/${id}`)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(`http://192.168.0.200:8080/endpoint/update-itemOut/${id}`, data)
      if (res) {
        handleUpdateInfo();
        handleQty()
        handleCreateComment()
