...
const PrintPayRoll = React.forwardRef(({id},ref) => {
    ...
    useEffect(()=> {
        axios.get('http://192.168.0.200:8080/endpoint/payRoll')
        .then(res => {
      ...
      useEffect(()=> {
        axios.get('http://192.168.0.200:8080/endpoint/employee')
                .then(res => {
                        // Handle the response data here
  ...
  return (
    <div>
           ...
           <thead>
             ...
             <tr>
               <th  style={{padding:'5px',border:'1px solid #DDD',backgroundColor:'#e8f7fe', color:'black'}} colSpan={5}>ÙSalary Slip for the month Of: {dayjs(row.month).format('MMMM-YYYY')}</th>
             </tr>
           </thead>
           <tbody>
          <tr>
