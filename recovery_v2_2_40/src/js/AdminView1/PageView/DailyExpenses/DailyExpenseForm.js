    ...
    const handleLogout = () => {
     ...
     useEffect(()=>{
      const fetchlastNumber = async () => {
       if (navigator.onLine) {
         try {
           const res = await axios.get('http://192.168.0.200:8080/endpoint/get-last-saved-expense')
      ...
      useEffect(()=> {
              ...
              if (navigator.onLine) {
                  try {
                    const res = await axios.get('http://192.168.0.200:8080/endpoint/paymentRate')
...
const handleSubmit = async (e) => {
    ...
    if (navigator.onLine) {
      try {
          const res = await axios.post('http://192.168.0.200:8080/endpoint/create-expense',data);
          if (res) {
              handleOpen();
