...
if (navigator.onLine) {
      try {
        const res = await axios.get('http://192.168.0.200:8080/endpoint/itemPurchase')    
        setItemPurchase(res.data.data.filter((row)=>row.status && row.status === "Paid"));
        const resPO = await axios.get('http://192.168.0.200:8080/endpoint/purchaseOrder')    
        setPurchaseOrder(resPO.data.data.filter((row)=> row.status && row.status === 'Purchase'));
        const expenseResponse = await axios.get('http://192.168.0.200:8080/endpoint/expense')  
        setExpenses(expenseResponse.data.data);
        const resPayment = await axios.get('http://192.168.0.200:8080/endpoint/payment')
        ...
        const rateResponse = await axios.get('http://192.168.0.200:8080/endpoint/rate') 
        rateResponse.data.data.map((row)=> setRate(row.rate))
        const resPayRoll = await axios.get('http://192.168.0.200:8080/endpoint/payRoll')
        setPayRoll(resPayRoll.data.data.filter((row)=>row.status !== undefined? row.status === 'Paid' :null))
        const resPos = await axios.get('http://192.168.0.200:8080/endpoint/pos')
   ...
   useEffect(()=>{
    const fetchId = async () => {
     if (viewId !== null) {
       if (navigator.onLine) {
        try {
          const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-cash/${viewId}`)
   ...
   useEffect(()=>{
    const fetchId = async () => {
     if (viewIdStatus !== null) {
      if (navigator.onLine) {
         try {
           const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-cash/${viewIdStatus}`)
           setCashDate(res.data.data.cashDate)
...
const handleSubmitUpdate = async(e) => {
 ...
 const data = {
  ...
  if (navigator.onLine) {
    try{
      const res = await axios.put(`http://192.168.0.200:8080/endpoint/update-cash/${viewId}`,data); 
      if (res) {
...
if (navigator.onLine) {
   try{
    const res = await axios.put(`http://192.168.0.200:8080/endpoint/update-cash/${viewIdStatus}`,data); 
    if (res) {
      await db.cashSchema.update(indexId,{...data, updateS: true})
      handleOpen();
    }
...
const newArrayCash = useMemo(()=>search2 !== ''?cash.filter((row)=>
  row.amount.some((Item)=> Item.note && Item.note.toLowerCase().includes(search2.toLowerCase())) ||
  dayjs(row.cashDate).format('DD/MM/YYYY').includes(search2)
   ):cash,[cash,search2])

const newArray = search !== ''?expensesFiltered.filter((row)=>
     row.expenseNumber.toString().includes(search)||
     row.accountName.toLowerCase().includes(search.toLowerCase())||
     row.expenseCategory.expensesCategory.toLowerCase().includes(search.toLowerCase()) ||
     row.accountNameInfo.name.toLowerCase().includes(search.toLowerCase()) ||
     row.employeeName.some((Item)=> Item.employee.toLowerCase().includes(search.toLowerCase())) 
      ):expensesFiltered
