...
function ProjectUpdateView() {
    ...
    useEffect (() => {
      ...
      if (navigator.onLine) {
          try {
         const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-projects/${id}`)
      ...
      fetchData()
        ...
        useEffect(()=>{
          const handleFetch = async () => {
           if (navigator.onLine) {
             try {
               const resCustomer = await axios.get('http://192.168.0.200:8080/endpoint/customer')
...
const handleUpdatePurchase = async () => {
    ...
    if (purchase !== null) {
       if (navigator.onLine) {
         try {
           await axios.put(`http://192.168.0.200:8080/endpoint/update-purchase/${purchase}`,data)
...
const handleSubmit =async (e)=>{
   ...
   if (navigator.onLine) {
     try{
       const res = await axios.put(`http://192.168.0.200:8080/endpoint/update-projects/${id}`,data);
       if (res) {
         // Open Loading View
