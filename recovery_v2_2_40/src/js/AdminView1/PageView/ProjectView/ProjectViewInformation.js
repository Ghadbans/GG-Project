...
function ProjectViewInformation() {
      ...
      const fetchDataHidden = async () => {
       if (navigator.onLine) {
         try {
           ...
           const resP = await axios.get(`http://192.168.0.200:8080/endpoint/get-projects/${id}`)
  ...
  useEffect(()=> {
    const fetchInvoice = async () => {
      if (navigator.onLine) {
        try {
          ...
          const NewData = res.data.data.filter((row)=> 
          ...
          const resIPurchase = await axios.get('http://192.168.0.200:8080/endpoint/itemPurchase')
          setItemPurchase(resIPurchase.data.data.filter((row)=> row.projectName && row.projectName._id === id ))
          ...
          const result = resExpenses.data.data.filter((row)=> row.accountNameInfo !== undefined && row.accountNameInfo._id === id)
                              .map((row,i)=>({
                                _id: row._id,
    ...
    setNotification(resNotification.data.data.filter((row)=> row.idInfo === id))
          const resPlaning = await axios.get('http://192.168.0.200:8080/endpoint/planing')
  ...
  const CommentInfo = 
    ...
    const handleSynced = async(e)=> {
      ...
      const updatePurchase = relatedPurchase.map((row)=>{
        return axios.put(`http://192.168.0.200:8080/endpoint/update-purchase/${row._id}`,{
          items:row.items
        })  
       }) 
       try {
    ...
    const updateForItemBuy = async (e) => {
      ...
      const saveAttendance = purchaseInfo.map((row)=>{
        return axios.put(`http://192.168.0.200:8080/endpoint/update-purchase/${row._id}`,row)
      })
      try {
