...
function SupplierFormUpdate() {
    ...
    useEffect(()=>{
        const fetchData = async () => {
         if (navigator.onLine) {
           try {
             const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-Supplier/${id}`)
      ...
      const handleSubmitEdit = async(e) => {
         ...
         if (navigator.onLine) {
           try {
             const res = await axios.put(`http://192.168.0.200:8080/endpoint/update-Supplier/${id}`,data)
             if (res) {
               handleCreateComment();
               handleOpen();
             }
