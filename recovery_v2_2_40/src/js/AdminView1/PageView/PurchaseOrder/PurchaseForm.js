...
import { MenuItem, Grid, IconButton, Paper, TextField, FormControl, InputLabel, Select, Typography, styled, Box, Autocomplete, Modal, Backdrop, TableContainer, OutlinedInput, InputAdornment, Divider, Checkbox, FormControlLabel } from '@mui/material'
...
function PurchaseForm() {
  ...
  useEffect(() => {
    const fetchNumber = async () => {
      if (navigator.onLine) {
        try {
          const resItemOut = await axios.get('http://192.168.0.200:8080/endpoint/get-last-saved-purchaseOrder')
  ...
  const handleAddToPurchaseOrder = (shopItem) => {
    ...
    if (existingItemIndex !== -1) {
      ...
      const newQty = parseInt(currentItem.qtyBuy) + 1;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('http://192.168.0.200:8080/endpoint/create-purchaseOrder', data);
      if (res) {
        // Open Loading View
                    ...
                    <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                      ...
                      <CardContent sx={{ flexGrow: 1, padding: '10px' }}>
                        ...
                        <Button
                          ...
                          size="small"
                          fullWidth
                          onClick={() => handleAddToPurchaseOrder(item)}
                          sx={{ marginTop: '5px' }}
                        >
                          Add
                        </Button>
