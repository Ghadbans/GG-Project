    ...
    const [inputValue, setInputValue] = React.useState('');
      ...
      useEffect(()=>{
        const handleFetch = async () => {
          try {
            ...
            const resItemOut = await axios.get('http://192.168.0.200:8080/endpoint/get-last-saved-itemPurchase')
      ...
      useEffect (() => {
        const fetchData = async () => {
          try {
         const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-purchaseOrder/${id}`)
      ...
      useEffect(()=>{
          const fetchPur = async () => {
            ...
            const resM = await axios.get('http://192.168.0.200:8080/endpoint/maintenance')
            setMaintenanceInfo(resM.data.data.filter((row)=>row._id === projectName._id));
          } else if (reason === 'Invoice') {
            const resM = await axios.get('http://192.168.0.200:8080/endpoint/invoice')
...
const onStatusUpdate = async (ReferenceInfo,ReferenceInfoNumber) => {
 ...
 await axios.put(`http://192.168.0.200:8080/endpoint/update-purchaseOrder/${id}`,data);
  } catch (error) {
    console.log(error)
  }
}
...
const [saving,setSaving] = useState('')
  ...
  const handleSubmit = async (e)=>{
    ...
    try{
      const res = await axios.post('http://192.168.0.200:8080/endpoint/create-itemPurchase',data);
      if (res) {
        // Open Loading View
