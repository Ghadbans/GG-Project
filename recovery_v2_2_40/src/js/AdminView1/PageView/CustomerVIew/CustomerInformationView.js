...
function CustomerInformationView() {
  ...
  const apiUrl = 'http://192.168.0.200:8080/endpoint/customer';
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (navigator.onLine) {
        try {
          const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-customer/${id}`)
          setCustomerInfo(res.data.data.Customer)
          const estimateResponse = await axios.get('http://192.168.0.200:8080/endpoint/estimation');
          setEstimate(estimateResponse.data.data.filter((row) => row.customerName._id === id).reverse());
          const invoiceResponse = await axios.get('http://192.168.0.200:8080/endpoint/invoice');
          ...
          const purChaseResponse = await axios.get('http://192.168.0.200:8080/endpoint/purchase');
          setPurchase(purChaseResponse.data.data.filter((row) => row.customerName._id === id).reverse());
          const maintenanceResponse = await axios.get('http://192.168.0.200:8080/endpoint/maintenance');
  ...
  const handleUpdateCredit = async (e) => {
    ...
    if (navigator.onLine) {
      try {
        const res = await axios.put(`http://192.168.0.200:8080/endpoint/update-customer/${id}`, data)
        if (res) {
          setIsCredit('true')
          handleOpen();
        }
