...
function ShopPosForm() {
  ...
  const apiUrl = 'http://192.168.0.200:8080/endpoint/create-invoice';
  ...
  const fetchItems = async () => {
    ...
    if (navigator.onLine) {
      try {
        const res = await axios.get(`http://192.168.0.200:8080/endpoint/item-shop?page=${page}&limit=48&search=${search}&sort=-_id`);
      ...
      const validItems = offLineItems.filter(item => item.typeItem === 'Goods').reverse();
  ...
  useEffect(() => {
    const fetchlastNumber = async () => {
      if (navigator.onLine) {
        try {
          const resRate = await axios.get('http://192.168.0.200:8080/endpoint/rateReturn')
          resRate.data.data.map((row) => setRate(row.rateR))
          const res = await axios.get('http://192.168.0.200:8080/endpoint/get-last-saved-pos')
  ...
  const handleSubmit = async (e) => {
    ...
    if (navigator.onLine) {
      try {
        const res = await axios.post('http://192.168.0.200:8080/endpoint/create-pos', data);
        if (res) {
