...
const handleCreateItem = (newItem)=> {
  ...
  useEffect(()=> {
    ...
    const handleSubmitStatusUpdate = async (ReferenceInfo) => {
        ...
        try {
          await axios.put(`http://192.168.0.200:8080/endpoint/update-maintenance/${id}`,data)
        } catch (error) {
          console.log(error)
        }
     }
    ...
    const data = {
      ...
      try{
        const res = await axios.post('http://192.168.0.200:8080/endpoint/create-invoice',data);
        if (res) {
