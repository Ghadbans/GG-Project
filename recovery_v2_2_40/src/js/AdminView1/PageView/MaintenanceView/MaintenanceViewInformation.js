  ...
  const [planingInfo,setPlaningInfo] = useState([]);
    useEffect(()=> {
      ...
      if (navigator.onLine) {
         ...
         setMaintenance(res.data.data.reverse());
        ...
        setMaintenance(offLineCustomer1.reverse());
      ...
      const getMaintenance = async () => {
        if (navigator.onLine) {
          try {
           ...
           const totalCostInfo = Number(totalAmountPlaning) + Number(costInfo)
...
const relatedPurchase = maintenance.filter((row)=> row._id === id).map((row)=>({
  ...row,
...
const handleSynced = async(e)=> {
  ...
  const updatePurchase = relatedPurchase.map((row)=>{
    return axios.put(`http://192.168.0.200:8080/endpoint/update-maintenance/${row._id}`,{
      items:row.items
    })  
   }) 
   try {
...
{/** Item Out sync end */}
    ...
    const newArray = search !== ''?maintenance.filter((row)=>
