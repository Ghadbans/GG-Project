...
function MaintenanceFormView() {
  ...
  const apiUrl = 'http://192.168.0.200:8080/endpoint/create-maintenance';
  ...
  useEffect(() => {
    const fetchlastNumber = async () => {
      if (navigator.onLine) {
        try {
          const res = await axios.get('http://192.168.0.200:8080/endpoint/get-last-saved-maintenance')
  ...
  const handleRefreshShop = () => {
    // Trigger re-fetch by toggling a dummy state or just ensure useEffect runs? 
    // Simplest is to just rely on useEffect dependencies.
  };
