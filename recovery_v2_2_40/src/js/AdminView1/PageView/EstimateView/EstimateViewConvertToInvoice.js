...
useEffect(()=> {
  const fetchCustomer = async ()=>{
   if (customerName) {
     try {
      const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-customer/${customerName._id}`)
...
{/** Item InFO End */}
     ...
     const [saving,setSaving] = useState('')
            const handleSubmit = async (e) =>{
              ...
              try {
                const res = await  axios.post('http://192.168.0.200:8080/endpoint/create-invoice', {
                  customerName,
                  invoiceNumber,
                  invoiceDate,
                  invoiceName,
...
export default EstimateViewConvertToInvoice
