...
function MakePurchaseConvertToProject() {
  ...
  useEffect (() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-estimation/${id}`)
        setCustomerNameInfo(res.data.data.customerName.customerName);
        SetItems(res.data.data.items);
        setInvoiceName(res.data.data.estimateName);
...
const handleCreateItem = (newItem)=> {
  ...
  const handleSubmitStatusUpdate = async (ReferenceInfo) => {
      ...
      try {
       await axios.put(`http://192.168.0.200:8080/endpoint/update-estimation/${id}`,data)
      } catch (error) {
        console.error('Error fetching data:', error);
      }
   }
  ...
  const [saving,setSaving] = useState('')
    const handleConvertInvoiceToPurchase = async (e) => {
      ...
      const data = {
        ...
        purchaseNumber,
        purchaseDate,
        items,noteInfo,estimateDefect,estimateSubject,
        description,
        ReferenceName:id,
        purchaseName,
        Position:'Second',
      ...
      try{
        const res = await axios.post('http://192.168.0.200:8080/endpoint/create-purchase',data);
        if (res) {
           // Open Loading View
       ...
       >
         ...
         >
             ...
             <div style={{justifyContent:'center',textAlign:'center'}}>
                 ...
                 <NavLink to={`/PurchasesViewAdminAll/${idInvoice}`} className='LinkName'>
    ...
    >
      ...
      <BlackTooltip title="Close" placement='left'>
        ...
        <div style={{height:'600px', padding:'20px',overflow:'hidden',overflowY:'scroll'}}>
            <ProjectFormView2 onCreateOption={handleCreateProject} onEstimate={id} onClose={handleCloseOpenAutocomplete1}/>  
        </div>
      </Box>
                </Modal>
                <Modal
...
export default MakePurchaseConvertToProject
