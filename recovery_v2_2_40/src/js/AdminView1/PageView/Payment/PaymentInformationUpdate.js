...
useEffect(()=>{
  const fetchlastNumber = async () => {
    try {
        const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-payment/${id}`)
       ...
       if (navigator.onLine) {
        try {
         ...
         if (customerId !== null) {
           ...
           const exInvoice = filterData?.filter((row)=> row.status === 'Sent' || row.status === 'Partially-Paid' & !invoice1.some((row1)=> row1.id === row._id  ) )
                        .map((row)=> ({
                          id: row._id,
...
useEffect(()=>{
  if (parseFloat(creditOld)=== parseFloat(remaining) ) {
    ...
    let totalR = creditOld - remaining
    const totalR1 = isNaN( oldCredit - totalR)?0:parseFloat(Number(oldCredit) - totalR )
    setCredit(totalR1)
  }else if (parseFloat(creditOld) < parseFloat(remaining)) {
    let totalR = remaining- creditOld
    const totalR1 = isNaN( Number(oldCredit) +  Number(totalR))?0:parseFloat(Number(oldCredit) + Number(totalR) )
    setCredit(totalR1)
  }
...
const TotalAmount =  invoice?.filter((row)=> parseFloat(row.total) !== 0)
const invoiceUpdate =  invoice?.map((row)=>{
  const correspondingId = oldInvoice?.find((row2)=> row2._id === row.id)
  let difference =0
  let total =0
   const difference1 =  parseFloat(row.balanceDue).toFixed(2)-parseFloat(row.total).toFixed(2)
   difference = difference1
  total = parseFloat(Number(correspondingId.totalInvoice) - Number(difference1)).toFixed(2)

  if (parseFloat(total) === parseFloat(correspondingId.totalInvoice) && parseFloat(total) !== 0 ) {
    row.status = 'Paid'
...
const handleSubmit = async (e) => {
  ...
  if (navigator.onLine) {
    try{
      const res = await axios.put(`http://192.168.0.200:8080/endpoint/update-payment/${id}`,data); 
      if (res) {
