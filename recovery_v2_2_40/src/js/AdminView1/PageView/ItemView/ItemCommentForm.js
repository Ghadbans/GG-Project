...
function ItemCommentForm({onCreateOption,onId}) {
  ...
  useEffect(()=> {
    ...
    useEffect (() => {
        axios.get(`http://192.168.0.200:8080/endpoint/get-item/${onId}`)
        .then(res => {
    ...
    const Comments = [ ...Comments1, CommentInfo];
     ...
     const fetchingData = async () => {
        try {
            const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-item/${onId}`)
     ...
     const handleSubmitEdit = (e) => {
      ...
      axios.put(`http://192.168.0.200:8080/endpoint/update-item/${onId}`,data)
       .then((res) => {
         if (res) {
           setReason("");
           handleOpen();
