...
function ItemInformationVIew() {
 ...
 const ItemInfoU = grantAccess.filter((row)=> row.moduleName === "Item" && row.access.editM === true);
  ...
  useEffect(()=>{
    const handleFetch = async () => {
      try {
        ...
        const resPosOut = await axios.get('http://192.168.0.200:8080/endpoint/pos')
        ...
        const formatDate2 = resIReturn.data.data.map((row)=>({
          ...
          setItemReturn(formatDate2.reverse());
  ...
  const fetchStock = async () => {
    try {
        const res = await axios.put(`http://192.168.0.200:8080/endpoint/update-item/${id}`,{
          itemQuantity:stock
        })
    } catch (error) {
      console.log(error)
    ...
    const apiUrl = 'http://192.168.0.200:8080/endpoint/item';
    useEffect(()=> {
  ...
  setAnchorEl(null);
    ...
    const [uploadedImageURL, setUploadedImageURL]= useState('')
        ...
        const handleImageSubmit = async (e)=> {
          ...
          try {
              await axios.put(`http://192.168.0.200:8080/endpoint/upload-image-item/${id}`, formData,{
              headers: {
...
useEffect(()=>{
 ...
 const newArray2 = search2 !== ''?newItemOut.filter((row)=>
 row.reason.toLowerCase().includes(search2.toLowerCase())||
 row.reference.referenceName.toLowerCase().includes(search2.toLowerCase()) ||
 row.itemsQtyArray.some((Item)=> Item.itemName.itemName.toLowerCase().includes(search2.toLowerCase())) ||
 row.itemsQtyArray.some((Item)=> Item.itemDescription.toLowerCase().includes(search2.toLowerCase())) ||
 dayjs(row.itemOutDate).format('DD/MM/YYYY').includes(search2)
  ):newItemOut
...
const handleSearch3 = (e) => {
 ...
 const newArray3 = search3 !== ''?itemReturn.filter((row)=>
 ...
 dayjs(row.itemOutDate).format('DD/MM/YYYY').includes(search3)
  ...
  useEffect(()=>{
    const fetchData2 = async () => {
     ...
     if (navigator.onLine) {
       try {
           const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-itemPurchase/${idView}`)
  ...
  useEffect(()=>{
    const fetchData2 = async () => {
      ...
      if (navigator.onLine) {
        try {
          const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-itemOut/${idView2}`)
