    ...
    const [items, SetItems] = useState([]);
      ...
      useEffect (() => {
        ...
        useEffect(()=> {
          const fetchCustomer = async ()=>{
           if (customer) {
             try {
              const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-customer/${customer._id}`)
               setCustomerName(
                {
      ...
      useEffect(()=>{
        const fetchlastNumber = async () => {
          try {
            const res = await axios.get('http://192.168.0.200:8080/endpoint/get-last-saved-invoice')
...
useEffect(()=> {
  ...
  const handleDecision = (navigate) => {
    ...
    const handleSubmitStatusUpdate =  async (ReferenceInfo) => {
        ...
        try {
          await  axios.put(`http://192.168.0.200:8080/endpoint/update-purchase/${id}`,data)
        } catch (error) {
          console.error(error)
        } 
     }
    const handleSubmitStatusProject =  async () => {
        ...
        try {
          await  axios.put(`http://192.168.0.200:8080/endpoint/update-projects/${projectId}`,data)
        } catch (error) {
          console.error(error)
        } 
     }
