 ...
 const PurchaseInfoD = grantAccess.filter((row)=> row.moduleName === "Item-Purchase" && row.access.deleteM === true);
    ...
    const handleLogout = () => {
      ...
      const fetchItems = async (page, searchTerm, filterField, filterValue) => {
        try {
          const res = await axios.get(`http://192.168.0.200:8080/endpoint/itemPurchase-Information?page=${page + 1}&limit=${limit}&search=${encodeURIComponent(searchTerm.trim())}&filterField=${encodeURIComponent(filterField.trim())}&filterValue=${encodeURIComponent(filterValue.trim())}`);
      ...
      const fetchAndSaveData = async () => {
        try {
          ...
          await db.itemPurchaseViewSchema.clear();
          await db.itemPurchaseViewSchema.bulkPut(res.data.data);
...
useEffect(()=>{
  const fetchId = async () => {
   if (DeleteId) {
     try {
       const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-itemPurchase/${DeleteId}`)
...
const onStatusUpdate = async () => {
  if (purChaseOrderId !== undefined) {
   ...
   await axios.put(`http://192.168.0.200:8080/endpoint/update-purchaseOrder/${purChaseOrderId}`,data);
    } catch (error) {
      console.log(error)
    }
  }
...
const handleDeleteUpdate = async (e) => {
  e.preventDefault()
  try {
    const res = await axios.delete(`http://192.168.0.200:8080/endpoint/delete-itemPurchase/${DeleteId}`);
    if (res) {
            onStatusUpdate();
            handleQty();
...
const onStatusUpdatePaid = async (e,idInfo) => {
  e.preventDefault();
    try {
      ...
      const res = await axios.put(`http://192.168.0.200:8080/endpoint/update-itemPurchase/${idInfo}`,data);
      if (res) {
        setUpdateS(true);
        handleDeleteOpenLoading();
      }
