...
function SupplierAdminView() {
 ...
 const [open1, setOpen1] = React.useState(true);
     ...
     const fetchData = async () => {
       if (navigator.onLine) {
         try {
           const res = await axios.get('http://192.168.0.200:8080/endpoint/Supplier')
           const formatDate = res.data.data.map((item)=>({
             ...item,
             id: item._id,
           })) 
           setCustomer(formatDate.reverse());
           await db.supplierSchema.clear();
           await Promise.all(res.data.data.map( async (item,i)=>{
            await db.supplierSchema.put({...item, id: i+1, synced: true,updateS:true})
           }))
       ...
       try {
         ...
         console.log(error)
           ...
           const syncOff = async () => {
             if (navigator.onLine) {
              ...
              for(const customers of customertoSynChro){
               try {
                 const res = await axios.post('http://192.168.0.200:8080/endpoint/create-Supplier',customers)
                if (res) {
               ...
               for(const customersUpdate of customertoSynChroUpdate){
                 try {
                    await axios.put(`http://192.168.0.200:8080/endpoint/update-Supplier/${customersUpdate._id}`,customersUpdate)
                    handleOpenOffline();
                 } catch (error) {
                   console.log(error)
                 }
           ...
           useEffect(()=>{
             const fetchFunction = async () => {
                 const deletePromises = selectedRows.map( async (idToDelete)=> {
                   return axios.get(`http://192.168.0.200:8080/endpoint/get-Supplier/${idToDelete}`)
                 })
                 try {
           ...
           const handleDeleteMany = async (e) => {
             ...
             if (navigator.onLine) {
               const deletePromises = selectedRows.map( async (idToDelete)=> {
                 return axios.delete(`http://192.168.0.200:8080/endpoint/delete-Supplier/${idToDelete}`)
               })
               try {
                 const res = await Promise.all(deletePromises);
                 if (res) {
