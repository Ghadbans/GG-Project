    ...
    const handleLogout = () => {
     ...
     const [page, setPage] = useState(0); // Initialize page state to 0 (0-based index)
      ...
      const fetchItems = async (page, searchTerm, filterField, filterValue) => {
        try {
          const res = await axios.get(`http://192.168.0.200:8080/endpoint/expense-Information?page=${page + 1}&limit=${limit}&search=${encodeURIComponent(searchTerm.trim())}&filterField=${encodeURIComponent(filterField.trim())}&filterValue=${encodeURIComponent(filterValue.trim())}`);
          const formatDate = res.data.itemI.map((item) => ({
            ...item,
            id: item._id,
            dataField: dayjs(item.expenseDate).format('DD/MM/YYYY'),
            category: item.expenseCategory.expensesCategory,
            name: item.accountNameInfo?item.accountNameInfo.name:'',
            employee: item.employeeName.map((row)=> row.employee)
          }));
      ...
      const fetchAndSaveData = async () => {
        try {
          ...
          await db.dailyExpenseViewSchema.clear();
          await db.dailyExpenseViewSchema.bulkPut(res.data.data);
          console.log('Data saved to IndexedDB successfully');
          const resRate = await axios.get('http://192.168.0.200:8080/endpoint/rate')
          resRate.data.data.map((row)=> setRate(row.rate))

          await Promise.all(resRate.data.data.map( async (item,i)=>{
           await db.rateSchema.put({...item,id:i+1, synced: true,updateS:true})
          }))

          const resPaymentRate = await axios.get('http://192.168.0.200:8080/endpoint/paymentRate')
          await Promise.all(resPaymentRate.data.data.map( async (item,i)=>{
           await db.paymentRateSchema.put({...item,id:i+1, synced: true,updateS:true})
          }))
          const resCash = await axios.get('http://192.168.0.200:8080/endpoint/cash')
          await db.cashSchema.clear();
           await Promise.all(resCash.data.data.map( async (item,i)=>{
                             await db.cashSchema.put({...item,cashNumber:i+1 ,synced: true,updateS:true})
                            }))
        const resCategory = await axios.get('http://192.168.0.200:8080/endpoint/expensesCategory')
        await Promise.all(resCategory.data.data.map( async (item,i)=>{
         await db.dailyExpensesCategorySchema.put({...item, id:i+1, synced: true,updateS:true})
        }))
        } catch (error) {
...
useEffect(()=>{
 ...
 useEffect(()=>{
    const fetchNumber = async () => {
     if (navigator.onLine) {
       try {
         const res = await axios.get('http://192.168.0.200:8080/endpoint/get-last-saved-cash')
...
const handleCloseReasonDelete = () => {
  setOpenReasonDelete(false);
    ...
    const handleSubmitCategory = async(e)=> {
      ...
      try{
        const res = await axios.post('http://192.168.0.200:8080/endpoint/create-expensesCategory',data); 
        if (res) {
          handleOpenModal();
        }
      }catch(error){
    ...
    const handleSubmitCash = async(e)=> {
      ...
      if (navigator.onLine) {
        try{
          const res = await axios.post('http://192.168.0.200:8080/endpoint/create-cash',data); 
          if (res) {
        ...
        useEffect(()=>{
          const fetchFunction = async () => {
            const deletePromises = selectedRows.map( async (idToDelete)=> {
              return axios.get(`http://192.168.0.200:8080/endpoint/get-expense/${idToDelete}`)
            })
            try {
    ...
    const handleDelete = async () => {
      try {
        const res = await axios.delete(`http://192.168.0.200:8080/endpoint/delete-expense/${DeleteId}`);
        if (res) {
          handleOpenModal();
        }
      } catch (error) {
    ...
    const handleDeleteMany = async (e) => {
      ...
      if (navigator.onLine) {
        const deletePromises = selectedRows.map( async (idToDelete)=> {
          return axios.delete(`http://192.168.0.200:8080/endpoint/delete-expense/${idToDelete}`)
        })
        try {
          const res =await Promise.all(deletePromises);
          if (res) {
...
if (navigator.onLine) {
      try {
          const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-expense/${idView}`)
