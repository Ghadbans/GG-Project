...
function MaintenanceViewAdmin() {
  ...
  const fetchItems = async (page, searchTerm, filterField, filterValue) => {
    try {
      const res = await axios.get(`http://192.168.0.200:8080/endpoint/maintenance-Information?page=${page + 1}&limit=${limit}&search=${encodeURIComponent(searchTerm.trim())}&filterField=${encodeURIComponent(filterField.trim())}&filterValue=${encodeURIComponent(filterValue.trim())}`);
  ...
  const fetchAndSaveData = async () => {
    try {
      ...
      await db.maintenanceViewSchema.clear();
      await db.maintenanceViewSchema.bulkPut(res.data.data);
  ...
  useEffect(() => {
    if (updateId !== null) {
      axios.get(`http://192.168.0.200:8080/endpoint/get-maintenance/${updateId}`)
        .then(res => {
  ...
  const handleSubmitUpdateStatus = async (e) => {
    ...
    try {
      const res = await axios.put(`http://192.168.0.200:8080/endpoint/update-maintenance/${updateId}`, data)
      if (res) {
        handleOpenLoading();
        handleCreateComment();
      }
  ...
  const handleDelete = async () => {
    try {
      const res = await axios.delete(`http://192.168.0.200:8080/endpoint/delete-maintenance/${DeleteId}`);
      if (res) {
  ...
  useEffect(() => {
    const fetchFunction = async () => {
      const deletePromises = selectedRows.map(async (idToDelete) => {
        return axios.get(`http://192.168.0.200:8080/endpoint/get-maintenance/${idToDelete}`)
      })
      try {
  ...
  const handleDeleteMany = async (e) => {
    e.preventDefault()
    const deletePromises = selectedRows.map(async (idToDelete) => {
      return axios.delete(`http://192.168.0.200:8080/endpoint/delete-maintenance/${idToDelete}`)
    })
    try {
      const res = await Promise.all(deletePromises);
      if (res) {
