 ...
 // const targetTime = dayjs().hour(7).minute(30)
  ...
  if (navigator.onLine) {
        try {
          ...
          const resExpenses = await axios.get('http://192.168.0.200:8080/endpoint/expense');
          setExpenses(resExpenses.data.data.filter(row=>dayjs(row.expenseDate).format('YYYY') === dayjs(date).format('YYYY')))
          const resItemPurchase = await axios.get('http://192.168.0.200:8080/endpoint/itemPurchase')
          setItemPurchase(resItemPurchase.data.data.filter(row=>dayjs(row.itemPurchaseDate).format('YYYY') === dayjs(date).format('YYYY') && (row.status === undefined || row.status === "Paid")))
          const resPayRoll = await axios.get('http://192.168.0.200:8080/endpoint/payRoll')
          setPayRoll(resPayRoll.data.data.filter(row=>dayjs(row.month).format('YYYY') === dayjs(date).format('YYYY')))
          const resPayment = await axios.get('http://192.168.0.200:8080/endpoint/payment')
          setPayment(resPayment.data.data.filter(row=>dayjs(row.paymentDate).format('YYYY') === dayjs(date).format('YYYY')).map((row)=>({
            ...row,
            amount: row.amount - row.remaining
          })))
          const resPosInvoice = await axios.get('http://192.168.0.200:8080/endpoint/pos')
          ...
          setInvoice(resInvoice.data.data.filter((row)=> row.status === 'Paid').map((row)=>({
            ...row,
            id: row._id,
          ...
          const resAttendance = await axios.get('http://192.168.0.200:8080/endpoint/employeeattendance')
...
setAttendance(offLineAttendance.filter((row)=>dayjs(new Date(row.timeIn)).format('DD/MM/YYYY') === dayjs(date).format('DD/MM/YYYY') && (row.observation === 'P' || row.observation === 'S' || row.observation === 'A' || row.observation === 'H') ))
  ...
  const fetchValue = async() => {
    try {
      const resItemOutItemReturn = await axios.get('http://192.168.0.200:8080/endpoint/item-usage')
...
const [open, setOpen] = React.useState(true);
  ...
  return (
    <div className='Homeemployee'>
      {user.loading?(<div style={{position:'relative',
  top: '40%',
  left: '50%',
  transform: 'translate(-50%, -50%)',
  boxShadow: 24,
