  ...
  const fetchData = async () => {
    if (navigator.onLine) {
      try {
        ...
        await db.posSchema.clear();
        await Promise.all(res.data.data.map( async (item)=>{
          await db.posSchema.put({...item, synced:true,updateS:true})
         }))
...
const syncOff = async () => {
 if (navigator.onLine) {
  ...
  for(const invoiceInfo of invoiceToSynced){
   try {
      const res = await axios.post('http://192.168.0.200:8080/endpoint/create-pos',invoiceInfo)
      if (res) {
   ...
   for(const invoiceInfoUpdate of invoiceToSyncedUpdate){
     try {
        await axios.put(`http://192.168.0.200:8080/endpoint/update-pos/${invoiceInfoUpdate._id}`,invoiceInfoUpdate)
...
{/** Delete Function */}
  ...
  try {
    const res = await axios.delete(`http://192.168.0.200:8080/endpoint/delete-pos/${DeleteId}`);
    if (res) {
  ...
  useEffect(()=>{
    const fetchFunction = async () => {
      const deletePromises = selectedRows.map( async (idToDelete)=> {
        return axios.get(`http://192.168.0.200:8080/endpoint/get-pos/${idToDelete}`)
      })
      try {
  ...
  useEffect(()=>{
    const fetchData2 = async () => {
     ...
     if (navigator.onLine) {
       try {
           const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-pos/${idView}`)
  ...
  const handleDeleteMany = async (e) => {
    e.preventDefault()
      const deletePromises = selectedRows.map( async (idToDelete)=> {
        return axios.delete(`http://192.168.0.200:8080/endpoint/delete-pos/${idToDelete}`)
      })
    try {
      const res = await Promise.all(deletePromises);
      if (res) {
...
useEffect (() => {
  const fetchId = async () => {
    if (updateId !== null) {
      try {
        const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-pos/${updateId}`)
