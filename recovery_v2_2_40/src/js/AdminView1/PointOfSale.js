   ...
   if (navigator.onLine) {
     try {
      ...
      resRate.data.data.map((row)=> setRate(row.rate))
       const res = await axios.get(`http://192.168.0.200:8080/endpoint/item-shop?page=${page}&limit=60&search=${encodeURIComponent(search)}`)
    ...
    SetItems(offLineCustomer1.filter((row)=> row.typeItem === "Goods").reverse())
    setLoadingData(false)
