  ...
  const fetchData = async () => {
    if (navigator.onLine) {
      try {
        ...
        await db.employeeSchema.clear();
        await Promise.all(res.data.data.map( async (item)=>{
          await db.employeeSchema.put({...item, synced: true,updateS:true})
         }))
       ...
       const syncOff = async () => {
         if (navigator.onLine) {
          ...
          for(const EmployeeInfo of EmployeeToSynced){
           try {
              const res = await axios.post('http://192.168.0.200:8080/endpoint/create-employee',EmployeeInfo)
             if (res) {
           ...
           for(const EmployeeInfoUpdate of EmployeeToSyncedUpdate){
             try {
                await axios.put(`http://192.168.0.200:8080/endpoint/update-employee/${EmployeeInfoUpdate._id}`,EmployeeInfoUpdate)
...
const handleDelete = async () => {
  try {
        ...
        useEffect(()=>{
          const fetchFunction = async () => {
            const deletePromises = selectedRows.map( async (idToDelete)=> {
              return axios.get(`http://192.168.0.200:8080/endpoint/get-employee/${idToDelete}`)
            })
            try {
   ...
   const handleDeleteMany = async (e) => {
    e.preventDefault()
          const deletePromises = selectedRows.map( async (idToDelete)=> {
            return axios.delete(`http://192.168.0.200:8080/endpoint/delete-employee/${idToDelete}`)
          })
    try {
      const res = await Promise.all(deletePromises);
      if (res) {
