    ...
    const handleLogout = () => {
     ...
     const ItemOInfoD = grantAccess.filter((row)=> row.moduleName === "Item-Out" && row.access.deleteM === true);
      ...
      const fetchItems = async (page, searchTerm, filterField, filterValue) => {
        try {
          const res = await axios.get(`http://192.168.0.200:8080/endpoint/itemOut-Information?page=${page + 1}&limit=${limit}&search=${encodeURIComponent(searchTerm.trim())}&filterField=${encodeURIComponent(filterField.trim())}&filterValue=${encodeURIComponent(filterValue.trim())}`);
          ...
          console.error('Error fetching data:', error);
          // Handle offline case
          const offLineItems = await db.itemOutSchema.toArray();
      ...
      const fetchAndSaveData = async () => {
        try {
          const res = await axios.get(`http://192.168.0.200:8080/endpoint/itemOut`);
          await db.itemOutSchema.clear();
          await db.itemOutSchema.bulkPut(res.data.data);
...
const [filterModel, setFilterModel] = React.useState({
  ...
  const handleFilter = (newModel) => {
   ...
   useEffect(()=>{
    const storedQuick = JSON.parse(localStorage.getItem('QuickFilterItemOutTst'))
    if (storedQuick) {
    ...
    useEffect(()=>{
      const fetchData2 = async () => {
        ...
        if (navigator.onLine) {
          try {
            const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-itemOut/${idView}`)
...
useEffect(()=>{
  const fetchId = async () => {
    if (DeleteId !== null) {
      try {
        const res = await  axios.get(`http://192.168.0.200:8080/endpoint/get-itemOut/${DeleteId}`)
  ...
  const fetchData = async () => {
    try {
      const [purChaseResponse,maintenanceResponse,invoiceResponse] = await Promise.all([
        axios.get('http://192.168.0.200:8080/endpoint/purchase'),
        axios.get('http://192.168.0.200:8080/endpoint/maintenance'),
...
const handleUpdatePurchase = () => {
  ...
  return axios.put(`http://192.168.0.200:8080/endpoint/update-purchase/${projectId}`,data)
}
const handleUpdateInvoice = () => {  
  ...
  return axios.put(`http://192.168.0.200:8080/endpoint/update-invoice/${invoiceId}`,data)
}
const handleUpdateMaintenance = () => { 
  ...
  return axios.put(`http://192.168.0.200:8080/endpoint/update-maintenance/${serviceId}`,data)
}
{/** Update Info end */}
{/** Delete Start */}
...
const handleDeleteUpdate = async (e) => {
  e.preventDefault()
  try {
    const res = await axios.delete(`http://192.168.0.200:8080/endpoint/delete-itemOut/${DeleteId}`);
    if (res) {
