...
function InvoiceViewAdmin() {
  ...
  const fetchItems = async (page, searchTerm, filterField, filterValue) => {
    try {
      const res = await axios.get(`http://192.168.0.200:8080/endpoint/invoice-Information?page=${page + 1}&limit=${limit}&search=${encodeURIComponent(searchTerm.trim())}&filterField=${encodeURIComponent(filterField.trim())}&filterValue=${encodeURIComponent(filterValue.trim())}`);
  ...
  const fetchAndSaveData = async () => {
    try {
      ...
      await db.invoiceViewSchema.clear();
      await db.invoiceViewSchema.bulkPut(res.data.data);
  ...
  const handleDelete = async () => {
    try {
      const res = await axios.delete(`http://192.168.0.200:8080/endpoint/delete-invoice/${DeleteId}`);
      if (res) {
  ...
  useEffect(() => {
    const fetchFunction = async () => {
      const deletePromises = selectedRows.map(async (idToDelete) => {
        return axios.get(`http://192.168.0.200:8080/endpoint/get-invoice/${idToDelete}`)
      })
      try {
  ...
  const handleDeleteMany = async (e) => {
    e.preventDefault()
    const deletePromises = selectedRows.map(async (idToDelete) => {
      return axios.delete(`http://192.168.0.200:8080/endpoint/delete-invoice/${idToDelete}`)
    })
    try {
      const res = await Promise.all(deletePromises);
      if (res) {
  ...
  useEffect(() => {
    const fetchId = async () => {
      if (updateId !== null) {
        try {
          const res = await axios.get(`http://192.168.0.200:8080/endpoint/get-invoice/${updateId}`)
  ...
  const handleSubmitUpdateStatus = async (e) => {
    ...
    try {
      const res = await axios.put(`http://192.168.0.200:8080/endpoint/update-invoice/${updateId}`, data)
      if (res) {
        handleCreateComment();
        handleOpenLoading();
      }
