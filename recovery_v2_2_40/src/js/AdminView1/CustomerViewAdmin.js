  ...
  const fetchData = async () => {
    if (navigator.onLine) {
      try {
        ...
        await db.customerSchema.clear();
        await Promise.all(res.data.data.map( async (item,i)=>{
         await db.customerSchema.put({...item, id: i+1, synced: true,updateS:true})
        }))
    ...
    try {
      ...
      console.log(error)
        ...
        const syncOff = async () => {
          if (navigator.onLine) {
           ...
           for(const customers of customertoSynChro){
            try {
              const res = await axios.post('http://192.168.0.200:8080/endpoint/create-customer',customers)
             if (res) {
            ...
            for(const customersUpdate of customertoSynChroUpdate){
              try {
                 await axios.put(`http://192.168.0.200:8080/endpoint/update-customer/${customersUpdate._id}`,customersUpdate)
                 handleOpenOffline();
              } catch (error) {
                console.log(error)
              }
        ...
        useEffect(()=>{
          const fetchFunction = async () => {
              const deletePromises = selectedRows.map( async (idToDelete)=> {
                return axios.get(`http://192.168.0.200:8080/endpoint/get-customer/${idToDelete}`)
              })
              try {
        ...
        const handleDeleteMany = async (e) => {
          ...
          if (navigator.onLine) {
            const deletePromises = selectedRows.map( async (idToDelete)=> {
              return axios.delete(`http://192.168.0.200:8080/endpoint/remove-customer/${idToDelete}`)
            })
            try {
              const res = await Promise.all(deletePromises);
              if (res) {
