...
const deleteRegex = /Route\.route\("\/delete-purchase\/:id"\)\.delete\(async \(req, res, next\) => \{[\s\S]*?Route\.route\("\/remove-purchase"\)\.delete/;
...
const newDelete = `Route.route("/delete-purchase/:id").delete(async (req, res, next) => {
    ...
    try {
        ...
        const invoiceLink = await invoiceSchema.findOne({ ReferenceName2: id });
