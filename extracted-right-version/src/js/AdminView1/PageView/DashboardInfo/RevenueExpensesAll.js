...
function RevenueExpensesAll({onMonth,onPayment,onPayRoll,onItemPurChase,onExpenses}) {
    ...
    filterMonthItemPurchase.forEach(row=>{
      ...
      const Revenue = monthArray?.sort((a,b)=> new Date(a.date) - new Date(b.date)).map((row)=>{
        ...
        Details: row.type === 'Item Purchase' ? ('Ref '+ row.number  + ' / ' + row.description ):'' + row.type === 'Expenses' ? ('Ref D-0'+ row.number + ' / '+ row.reason + ' / ' + row.description ):'' + row.type === 'Pay Slip' ? ( row.description + ' REf PAY-0' + row.number):'' + row.type === 'Payment' && row.numberArray.length === 0 && row.credit > 0? ('PAY-0'+ row.number+ ' $' + row.credit + ' In Advanced Payment (Credit) '):('PAY-0'+ row.number+ ' $' + row.payment + ' for payment of ' + row.numberArray?.map((row2)=> 'INV-0'+row2.Ref)+ ' / Mode: ' + row.defect),
      ...
      const allStandingRow = monthArray?.sort((a,b)=> new Date(a.date) - new Date(b.date)).map((row,i)=>{
        ...
        <tr key={i}>
          ...
          <span>{row.type === 'Item Purchase' && ('Ref '+ row.number  + ' / ' + row.description )}</span>
